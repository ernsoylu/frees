...
- **REPL Terminal & Workspace**: A dockable, interactive console window (movable like the Editor and Variable Explorer) that evaluates one line at a time against the live workspace — a unit-aware calculator with history and Tab-completion. Query and assign variables, build matrices/ranges, solve a single unknown implicitly, run the full `CALL` library (with output lengths sized automatically so you can write bare output names), and use the embedded **Symja** CAS interactively (`Factor`, `Expand`, `Simplify`, `Together`, `Cancel`, `Collect`, `Diff`, `Integrate`, `Apart`, `Laplace`, `InverseLaplace`). A industry-standard-style variable explorer lists the workspace alongside it.
...
- **Display unit systems (Preferences)** — values are always solved in SI and converted for display only (the display-only conversion model): SI base, Engineering SI (kPa/kJ/kW), or US English (psi/Btu/hp/lbf/lbm/ft). A variable explicitly declared in a unit (e.g. `bar`) displays in that unit.
...
- **Story 11.3: Conditional & Series Functions.** Implement a reference solver-style `If(A, B, x, y, z)` conditional-value selection and `Product(i, lower, upper, term)` series (mirroring the existing `Sum` series form), integrated with array expansion and block ordering so conditionals resolve before the simultaneous blocks that consume them.
...
- **Story 11.8: Solid & Incompressible Material Properties.** Add an incompressible-substance/solid-material property source backing a reference solver's underscore-suffixed functions: `c_`, `k_`, `rho_`, `mu_`, `Pv_`, `E_` (Young's modulus), `nu_` (Poisson's ratio), `epsilon_` (emissivity), `VolExpCoef`, `FreezingPt`, `DELTAL\L_293` (linear expansion), and the Lennard-Jones parameters `ek_LJ`/`sigma_LJ`.
...
- **Milestone 11:** *Working software where every function listed in the* the standard methodology text *appendix either evaluates in frees or has a documented web-native equivalent — verified by an automated coverage test — so an engineer porting an a reference solver model finds its hyperbolic, rounding, conditional, statistical, lookup, string, and full thermophysical-property calls all resolve, each scalar intrinsic differentiates correctly inside the solver, and the Help page documents the complete function set.*

### Epic 12: Control Systems Design & Analysis

*Goal: Bring a matrix language-Control-Toolbox-style workflows — LTI modeling, interconnection, time/frequency analysis, and state-feedback/PID controller design — into frees as native, order-independent equations with interactive plots. Two complementary engines meet at the `num[]`/`den[]` coefficient arrays: the embedded **Symja** CAS for symbolic work (Laplace fractions, partial fractions/residues, symbolic `ss↔tf`), and **Apache Commons Math** (companion-matrix eigenvalues, Riccati via the matrix sign function) for numeric analysis that stays robust on high-order, floating-point systems. Models are plain array/matrix variables (no new data type): a TF is `num[]`/`den[]`, a SS model is `A`, `B`, `C`, `D`, a ZPK model is `z[]`/`p[]`/`k`. Multi-output routines reuse the existing `CALL Name(inputs : outputs)` dispatch, and new diagram types (`bode`, `nyquist`, `polezero`) are added purely as frontend `PlotSpec.kind`s. Each story is an independently shippable vertical slice: backend solver + unit tests first, catalog registration, a runnable `examples.ts` preset, and clean `./gradlew test` + `npm run build`.*

- **Story 12.0: Symbolic CAS Layer & Partial Fractions. [Implemented]** Embed the pure-Java **Symja** CAS in a `com.frees.backend.cas` package (`CasEngine` `factor`/`expand`/`simplify`/`apart`, `TransferFunction`, `CasIdentity`). Add the **`SYMBOLIC s`** keyword: an equation containing a symbolic variable is treated as an identity that must hold for all its values, routed through the CAS to emit `name = value` residue equations so partial-fraction residues become ordinary solved variables. `tf(num, den)` shorthand renders a Laplace fraction in LaTeX.
- **Story 12.1: LTI Models & Conversions. [Implemented]** Document the array/matrix conventions for TF, SS, and ZPK models. Implement a `PolynomialHelpers` utility (add/multiply, companion-matrix roots, `expandRoots`) and the conversions `tf2ss`, `ss2tf` (round-trip), `zp2tf`, `tf2zp` via `CALL` dispatch with automatic output-shape registration. Register `db` as a dimensionless unit and ground synthetic LTI conversion calls as dimensionless in the unit checker.
- **Story 12.2: System Interconnection. [Implemented]** Implement raw polynomial math (`multiplyRaw`/`addRaw`, preserving array lengths) and the `series`, `parallel`, and `feedback` connection routines, wired as `CALL` dispatches and grounded dimensionless.
- **Story 12.3: Frequency Analysis & Poles/Zeros. [Implemented]** Implement `bode` (magnitude dB + unwrapped phase), `nyquist` (real/imag), `margin` (gain/phase margins and crossover frequencies), and `pole`/`zero` for both TF and SS inputs. Add frontend `PlotSpec.kind`s `bode` (stacked dual-axis), `nyquist` (with the `-1 + j0` critical point), and `polezero` (s-plane scatter).
- **Story 12.4: Time-Domain Responses. [Implemented]** Implement `step`, `impulse`, and `lsim` for TF and SS models. Responses route through the tested `OdeIntegrator` (`integrateAndSampleAt` integrates the IVP once and samples at the requested time vector via Hermite dense output); a `TimeResponse` helper converts a TF to controllable canonical state space and integrates `x' = Ax + Bu`, `y = Cx + Du`. Plots reuse the existing **xy** kind.
- **Story 12.5: Controller Design Solvers. [Implemented]** Implement `lqr(A, B, Q, R)` (single-input continuous-time LQR — algebraic Riccati equation solved via the **matrix sign function** of the Hamiltonian → `K = R⁻¹B'P`), `place(A, B, pr, pi)` (SISO **Ackermann** pole placement, complex-conjugate poles supported), and `pidtune(num, den, type$, wc)` (closed-form loop-shaping for a P/PI/PID controller hitting a 60° phase margin at the target crossover, reusing `margin`).
- **Story 12.6: Polish, Docs & Presets. [Implemented]** Round out the **Control Systems** catalog category (catalog-driven highlighting/autocomplete cover every function); expand the Help **Control Systems & Symbolic CAS** topic across models, conversions, interconnection, frequency/time analysis, and design; and curate `examples.ts` presets spanning all phases, including an end-to-end **Control Analysis Report** that embeds Bode/Nyquist/pole-zero/step plots inline via `[Graph='…']` tags in the Formatted view.
- **Milestone 12:** *Working software where an engineer can model an LTI system as plain equations, interconnect blocks, read its poles/zeros, stability margins, and Bode/Nyquist/step responses as live plots, and synthesize an LQR, pole-placement, or PID controller — all as order-independent equations, with the analysis composable into a Formatted engineering report.*
### Epic 15: Asynchronous Compute Architecture (RabbitMQ & Redis)

*Goal: Decouple heavy computational workloads from the main API thread and enable horizontal scaling of compute nodes.*

- **API Node & Compute Node Profiles:** The backend is split into `api` and `compute` Spring profiles. The API node handles HTTP requests, performs synchronous syntax/shape validation, and enqueues tasks. The Compute node runs as a headless worker that consumes tasks.
- **RabbitMQ Task Queue:** Solves, optimization, and curve-fitting tasks are packaged as `ComputeTask` payloads and dispatched to a RabbitMQ queue (`frees.tasks`) with fair dispatching (`prefetch = 1`).
- **Redis Job & State Store:** `SolveContextCache` is migrated to Redis. Job status (`PENDING`, `COMPLETED`, `FAILED`) and final output payloads are stored in Redis under `job:<jobId>`. The REPL session state is mirrored to Redis under `session:<sessionId>`.
- **OpenTelemetry Distributed Tracing:** OpenTelemetry context is injected into RabbitMQ message headers by the producer and extracted by the consumer to enable unified distributed HTTP-to-MQ tracing with Jaeger.
- **Frontend Asynchronous Polling:** The frontend submits tasks via `/api/solve` (receiving `202 Accepted`) and polls `/api/jobs/{jobId}` until completion, seamlessly rendering the results.
- **Milestone 15:** *Working software where compute-heavy solve/optimization jobs are distributed horizontally across RabbitMQ worker nodes, state is persisted in Redis, and requests are fully trace-observable via OpenTelemetry.*
