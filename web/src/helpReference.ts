...
export const MATH_FUNCTIONS: FuncGroup[] = [
  ...
  },
  {
    ...
    blurb: 'If is the inline branch for the declarative top level; use IF…THEN inside function bodies.',
    functions: [
...
// ── Multi-output functions (control systems & linear algebra) ─────────────────
...
export const CALL_PROCEDURES: CallEntry[] = [
  { name: 'ss2tf', category: 'Model', desc: 'State space → transfer function', signature: '[num, den] = ss2tf(A, B, C, D)' },
  { name: 'tf2ss', category: 'Model', desc: 'Transfer function → state space', signature: '[A, B, C, D] = tf2ss(num, den)' },
  { name: 'zp2tf', category: 'Model', desc: 'Zero-pole-gain → transfer function', signature: '[num, den] = zp2tf(zr, zi, pr, pi, k)' },
  { name: 'tf2zp', category: 'Model', desc: 'Transfer function → zero-pole-gain', signature: '[zr, zi, pr, pi, k] = tf2zp(num, den)' },
  { name: 'series', category: 'Model', desc: 'Cascade two systems (G = G1·G2)', signature: '[num,den] = series(num1,den1,num2,den2)' },
  { name: 'parallel', category: 'Model', desc: 'Parallel sum (G = G1 + G2)', signature: '[num,den] = parallel(num1,den1,num2,den2)' },
  { name: 'feedback', category: 'Model', desc: 'Close a loop (T = G1/(1+G1·G2))', signature: '[num,den] = feedback(num1,den1,num2,den2)' },
  { name: 'pade', category: 'Model', desc: 'Padé approximation of a dead time', signature: '[num, den] = pade(Td, order)' },
  { name: 'ss2ss', category: 'Model', desc: 'Similarity transform x = P·z', signature: '[An,Bn,Cn,Dn] = ss2ss(A,B,C,D,P)' },
  { name: 'pole', category: 'Analysis', desc: 'System poles', signature: '[pr, pi] = pole(num, den)' },
  { name: 'zero', category: 'Analysis', desc: 'System zeros', signature: '[zr, zi] = zero(num, den)' },
  { name: 'bode', category: 'Analysis', desc: 'Bode magnitude (dB) & phase (deg)', signature: '[mag, phase] = bode(num, den, omega)' },
  { name: 'nyquist', category: 'Analysis', desc: 'Nyquist real/imaginary parts', signature: '[re, im] = nyquist(num, den, omega)' },
  { name: 'nichols', category: 'Analysis', desc: 'Nichols magnitude/phase', signature: '[mag, phase] = nichols(num, den, omega)' },
  { name: 'margin', category: 'Analysis', desc: 'Gain & phase margins + crossovers', signature: '[gm, pm, w_cg, w_cp] = margin(num, den)' },
  { name: 'rlocus', category: 'Analysis', desc: 'Root-locus trajectories over K', signature: '[K, cpr, cpi] = rlocus(num, den)' },
  { name: 'routh', category: 'Analysis', desc: 'Routh-Hurwitz: # RHP poles', signature: '[nRHP, stable] = routh(den)' },
  { name: 'errorconst', category: 'Analysis', desc: 'Static error constants Kp, Kv, Ka', signature: '[Kp, Kv, Ka] = errorconst(num, den)' },
  { name: 'ctrb', category: 'Analysis', desc: 'Controllability matrix', signature: '[Co] = ctrb(A, B)' },
  { name: 'obsv', category: 'Analysis', desc: 'Observability matrix', signature: '[Ob] = obsv(A, C)' },
  { name: 'gram', category: 'Analysis', desc: "Controllability ('c') / observability ('o') gramian", signature: "[W] = gram(A, M, 'c')" },
  { name: 'rank', category: 'Analysis', desc: 'Numerical matrix rank (SVD)', signature: '[r] = rank(M)' },
  { name: 'step', category: 'Analysis', desc: 'Unit step response', signature: '[y] = step(num, den, t)' },
  { name: 'impulse', category: 'Analysis', desc: 'Impulse response', signature: '[y] = impulse(num, den, t)' },
  { name: 'lsim', category: 'Analysis', desc: 'Response to arbitrary input u(t)', signature: '[y] = lsim(num, den, u, t)' },
  { name: 'stepinfo', category: 'Analysis', desc: 'Step metrics (Tr, Tp, Ts, OS)', signature: '[Tr, Tp, Ts, OS] = stepinfo(t, y)' },
  { name: 'mason', category: 'Analysis', desc: "Mason's gain formula for a graph", signature: '[T] = mason(G, source, sink)' },
  { name: 'lqr', category: 'Design', desc: 'LQR optimal state-feedback gain', signature: '[K] = lqr(A, B, Q, R)' },
  { name: 'dlqr', category: 'Design', desc: 'Discrete-time LQR gain (DARE)', signature: '[K] = dlqr(A, B, Q, R)' },
  { name: 'place', category: 'Design', desc: 'Pole placement (Ackermann)', signature: '[K] = place(A, B, pr, pi)' },
  { name: 'acker', category: 'Design', desc: "Pole placement (Ackermann's formula)", signature: '[K] = acker(A, B, pr, pi)' },
  { name: 'lqe', category: 'Design', desc: 'Kalman estimator (LQE) gain', signature: '[L] = lqe(A, G, C, Q, R)' },
  { name: 'balreal', category: 'Design', desc: 'Balanced realization (model reduction)', signature: '[Ab, Bb, Cb] = balreal(A, B, C)' },
  { name: 'pidtune', category: 'Design', desc: 'PID auto-tune by loop shaping', signature: "[Kp, Ki, Kd] = pidtune(num, den, 'PID', wc)" },
  { name: 'c2d', category: 'Digital', desc: 'Continuous → discrete (Tustin/ZOH)', signature: "[numz, denz] = c2d(num, den, Ts, 'zoh')" },
  { name: 'd2c', category: 'Digital', desc: 'Discrete → continuous (Tustin)', signature: "[num, den] = d2c(numz, denz, Ts, 'tustin')" },
  { name: 'residue', category: 'Linear', desc: 'Partial-fraction residues & poles', signature: '[rr, ri, pr, pi, k] = residue(num, den)' },
  { name: 'lyap', category: 'Linear', desc: "Continuous Lyapunov solve A·X+X·A'+Q=0", signature: '[X] = lyap(A, Q)' },
  { name: 'dlyap', category: 'Linear', desc: "Discrete Lyapunov (Stein) solve A·X·A'−X+Q=0", signature: '[X] = dlyap(A, Q)' },
  { name: 'dare', category: 'Linear', desc: 'Discrete algebraic Riccati solution', signature: '[X] = dare(A, B, Q, R)' },
  { name: 'Eigenvalues', category: 'Linear', desc: 'Matrix eigenvalues', signature: '[lambda] = Eigenvalues(A)' },
  { name: 'Eigen', category: 'Linear', desc: 'Eigenvalues & eigenvectors', signature: '[lambda, V] = Eigen(A)' },
  { name: 'LUDecompose', category: 'Linear', desc: 'LU decomposition', signature: '[L, U] = LUDecompose(A)' },
  { name: 'EulerRotate', category: 'Linear', desc: '3×3 rotation from Euler angles', signature: '[R] = EulerRotate(phi, theta, psi)' },
];
