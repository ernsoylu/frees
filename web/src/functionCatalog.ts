...
export const FUNCTION_CATEGORIES: FunctionCategory[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'residue (partial-fraction / inverse Laplace)', snippet: '[$0r_r, r_i, p_r, p_i, k] = residue(num, den)', description: 'Partial-fraction (Heaviside) expansion of num/den: residues (r_r, r_i), the matching poles (p_r, p_i), and the scalar direct term k, so that num/den = sum r_i/(s - p_i)^ord + k. The numeric inverse Laplace path — residues appear in the Solution window. Add a 6th output ord (CALL residue(num, den : r_r, r_i, p_r, p_i, ord, k)) to handle repeated poles, where ord is the power of each (s-p) term.', usage: '[r_r, r_i, p_r, p_i, k] = residue(num[1:1], den[1:3])' },
