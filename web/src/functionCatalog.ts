...
export const FUNCTION_CATEGORIES: FunctionCategory[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Eigenvalues', snippet: 'Eigenvalues($0)', description: 'Eigenvalues of a square matrix; [re, im] = Eigenvalues(A) returns a complex spectrum as real/imaginary parts.', usage: 'lambda = Eigenvalues(A)' },
      ...
      { label: 'EulerRotate', snippet: '[R] = EulerRotate($0phi, theta, psi)', description: '3D rotation matrix from Euler angles φ, θ, ψ (ZYX convention, radians). Output R is a 3×3 matrix.', usage: '[R] = EulerRotate(phi, theta, psi)' },
    ],
  },
  {
    category: 'Strings',
  ...
  },
  {
    ...
    items: [
      ...
      { label: '[a, b] = f(x) (multi-call / destructuring)', snippet: '[$0a, b] = fname(x)', description: 'array-language-style destructuring. Works for user multi-output FUNCTIONs AND every built-in multi-output function (e.g. [A, B, C, D] = tf2ss(num, den)). Use ~ to discard a slot, or omit trailing outputs you do not need.', usage: '[small, large] = Order(8, 3)\n[A, B, C, D] = tf2ss(num, den)\n[~, ~, V] = svd(M)\n[A, B] = tf2ss(num, den)   { C, D dropped }' },
      { label: 'Scalar function block', snippet: 'function y = $0pname(x)\n  y := \nend\n', description: 'Define a scalar function with ordered inputs and one output.', usage: 'function y = p(x) … end' },
      { label: 'Equation function block', snippet: 'function y = $0mname(x)\n  y = \nend\n', description: 'Define a reusable function of simultaneous equations.', usage: 'function y = m(x) … end' },
      ...
      { label: 'IF / THEN / ELSE (in FUNCTION)', snippet: 'IF $0 THEN\n  \nELSE\n  \nEND\n', description: 'Conditional branch inside a FUNCTION body.', usage: 'IF cond THEN … ELSE … END' },
      { label: 'REPEAT / UNTIL (in FUNCTION)', snippet: 'REPEAT\n  $0\nUNTIL ', description: 'Loop until a condition holds inside a FUNCTION.', usage: 'REPEAT … UNTIL cond' },
      { label: 'WHILE / DO (in FUNCTION)', snippet: 'WHILE $0 DO\n  \nEND\n', description: 'While-loop inside a FUNCTION.', usage: 'WHILE cond DO … END' },
    ],
  },
  {
    ...
    items: [
      ...
      { label: 'ss2tf (state-space to transfer function)', snippet: '[$0num, den] = ss2tf(A, B, C, D)', description: 'Convert state-space matrices A, B, C, D to transfer function coefficients num and den.', usage: '[num, den] = ss2tf(A, B, C, D)' },
      { label: 'tf2ss (transfer function to state-space)', snippet: '[$0A, B, C, D] = tf2ss(num, den)', description: 'Convert transfer function coefficients num and den to controllable canonical state-space matrices A, B, C, D.', usage: '[A, B, C, D] = tf2ss(num, den)' },
      { label: 'zp2tf (zero-pole-gain to transfer function)', snippet: '[$0num, den] = zp2tf(zr, zi, pr, pi, k)', description: 'Convert zero-pole-gain (zeros zr + j*zi, poles pr + j*pi, gain k) to transfer function coefficients num and den.', usage: '[num, den] = zp2tf(zr, zi, pr, pi, k)' },
      { label: 'tf2zp (transfer function to zero-pole-gain)', snippet: '[$0zr, zi, pr, pi, k] = tf2zp(num, den)', description: 'Convert transfer function coefficients num and den to zeros zr + j*zi, poles pr + j*pi, and gain k.', usage: '[zr, zi, pr, pi, k] = tf2zp(num, den)' },
      { label: 'series (series connection of systems)', snippet: '[$0num, den] = series(num1, den1, num2, den2)', description: 'Connect two transfer functions in series (multiplies their transfer functions).', usage: '[num, den] = series(num1, den1, num2, den2)' },
      { label: 'parallel (parallel connection of systems)', snippet: '[$0num, den] = parallel(num1, den1, num2, den2)', description: 'Connect two transfer functions in parallel (adds their transfer functions).', usage: '[num, den] = parallel(num1, den1, num2, den2)' },
      { label: 'feedback (feedback connection of systems)', snippet: '[$0num, den] = feedback(num1, den1, num2, den2, sign)', description: 'Connect two transfer functions in a feedback loop. sign is optional: 1.0 (default) for negative feedback, -1.0 for positive feedback.', usage: '[num, den] = feedback(num1, den1, num2, den2)' },
      { label: 'pole (system poles)', snippet: '[$0pr, pi] = pole(num, den)', description: 'Compute system poles (real part pr, imaginary part pi) for a transfer function or state space matrix A.', usage: '[pr, pi] = pole(num, den) or [pr, pi] = pole(A)' },
      { label: 'zero (system zeros)', snippet: '[$0zr, zi] = zero(num, den)', description: 'Compute system zeros (real part zr, imaginary part zi) for a transfer function or state space system (A, B, C, D).', usage: '[zr, zi] = zero(num, den) or [zr, zi] = zero(A, B, C, D)' },
      { label: 'bode (Bode frequency response)', snippet: '[$0mag, phase] = bode(num, den, omega)', description: 'Compute Bode frequency response magnitude (dB) and unwrapped phase (deg) for a transfer function or state space system at given frequencies omega.', usage: '[mag, phase] = bode(num, den, omega)' },
      { label: 'nyquist (Nyquist frequency response)', snippet: '[$0real, imag] = nyquist(num, den, omega)', description: 'Compute Nyquist frequency response real and imaginary parts for a transfer function or state space system at given frequencies omega.', usage: '[real, imag] = nyquist(num, den, omega)' },
      { label: 'margin (gain and phase margins)', snippet: '[gm, pm, w_cg, w_cp] = margin(num, den)', description: 'Compute closed-loop gain margin (dB), phase margin (deg), and crossover frequencies (gain w_cg, phase w_cp) for a transfer function or state space system.', usage: '[gm, pm, w_cg, w_cp] = margin(num, den)' },
      { label: 'step (unit step response)', snippet: '[$0y] = step(num, den, t)', description: 'Compute the unit step response y(t) for a transfer function or state space system at given time points t. Outputs are time/value arrays suitable for xy plotting.', usage: '[y] = step(num[1:3], den[1:3], t[1:100])' },
      { label: 'impulse (impulse response)', snippet: '[$0y] = impulse(num, den, t)', description: 'Compute the impulse response y(t) for a transfer function or state space system at given time points t. Outputs are time/value arrays suitable for xy plotting.', usage: '[y] = impulse(num[1:3], den[1:3], t[1:100])' },
      { label: 'lsim (linear simulation)', snippet: '[$0y] = lsim(num, den, u, t)', description: 'Simulate the output y(t) of a linear system driven by an arbitrary input signal u(t) at given time points t. Input u must be the same size as t.', usage: '[y] = lsim(num[1:3], den[1:3], u[1:100], t[1:100])' },
      { label: 'lqr (LQR optimal gain)', snippet: '[$0K] = lqr(A, B, Q, R)', description: 'Continuous-time LQR: optimal state-feedback gain K minimizing the quadratic cost with state weight Q and input weight R, solving the algebraic Riccati equation. Single-input: A and Q are n x n, B is an n-vector, R is a scalar, K is an n-vector.', usage: '[K] = lqr(A[1:2,1:2], B[1:2], Q[1:2,1:2], R)' },
      { label: 'dlqr (discrete LQR gain)', snippet: '[$0K] = dlqr(A, B, Q, R)', description: 'Discrete-time LQR: optimal state-feedback gain K for a discrete system, solving the discrete algebraic Riccati equation (DARE). A and Q are n x n, B is n x m, R is m x m, and K is m x n.', usage: '[K] = dlqr(A[1:2,1:2], B[1:2,1:1], Q[1:2,1:2], R)' },
      { label: 'place (pole placement)', snippet: '[$0K] = place(A, B, pr, pi)', description: 'SISO pole placement (Ackermann): state-feedback gain K that moves the closed-loop poles of (A - B K) to the requested locations, given as real/imag arrays pr, pi (each length n).', usage: '[K] = place(A[1:2,1:2], B[1:2], pr[1:2], pi[1:2])' },
      { label: 'acker (pole placement, Ackermann)', snippet: '[$0K] = acker(A, B, pr, pi)', description: "SISO pole placement via Ackermann's formula (alias of place): state-feedback gain K placing the closed-loop poles of (A - B K) at the locations given by the real/imag arrays pr, pi (each length n).", usage: '[K] = acker(A[1:2,1:2], B[1:2], pr[1:2], pi[1:2])' },
      { label: 'lqe (Kalman estimator gain)', snippet: '[$0L] = lqe(A, G, C, Q, R)', description: "Continuous-time Kalman estimator (LQE) gain L for the plant x' = A x + B u + G w, y = C x + v, with process-noise covariance Q and measurement-noise covariance R. Solves the filter Riccati equation; A is n x n, G is n x g, C is p x n, and L is n x p.", usage: '[L] = lqe(A[1:2,1:2], G[1:2,1:2], C[1:1,1:2], Q[1:2,1:2], R)' },
      { label: 'pidtune (auto-tune PID)', snippet: "[Kp, Ki, Kd] = pidtune(num, den, '$0', wc)", description: "Auto-tune a P/PI/PID controller for plant num/den with gain crossover at frequency wc and a 60-degree phase-margin target. Type is a quoted 'P', 'PI', or 'PID'. Unused gains are returned as 0.", usage: "[Kp, Ki, Kd] = pidtune(num[1:3], den[1:3], 'PID', wc)" },
      { label: 'ctrb (controllability matrix)', snippet: '[$0Ctrb] = ctrb(A, B)', description: 'Compute the controllability matrix Ctrb = [B, A*B, A^2*B, ..., A^(n-1)*B] for state-space matrices A (n x n) and B (n x m). Output Ctrb is n x (n*m).', usage: '[Ctrb] = ctrb(A[1:3,1:3], B[1:3])' },
      { label: 'obsv (observability matrix)', snippet: '[$0Obsv] = obsv(A, C)', description: 'Compute the observability matrix Obsv = [C; C*A; C*A^2; ...; C*A^(n-1)] for state-space matrices A (n x n) and C (p x n). Output Obsv is (n*p) x n.', usage: '[Obsv] = obsv(A[1:3,1:3], C[1:3])' },
      { label: 'gram (controllability/observability gramian)', snippet: "[W] = gram(A, M, '$0')", description: "System gramian via the Lyapunov equation. Type is a quoted 'c' (controllability gramian, with M = B) or 'o' (observability gramian, with M = C). Requires a stable A; the gramian W is n x n.", usage: "[W] = gram(A[1:2,1:2], B[1:2,1:1], 'c')" },
      { label: 'balreal (balanced realization)', snippet: '[$0Ab, Bb, Cb] = balreal(A, B, C)', description: 'Internally-balanced realization of a stable, minimal system (A, B, C): returns transformed matrices Ab, Bb, Cb whose controllability and observability gramians are equal and diagonal (the Hankel singular values). Useful for model reduction.', usage: '[Ab, Bb, Cb] = balreal(A[1:2,1:2], B[1:2,1:1], C[1:1,1:2])' },
      { label: 'dare (discrete Riccati solution)', snippet: '[$0X] = dare(A, B, Q, R)', description: 'Solve the discrete-time algebraic Riccati equation (DARE), returning the stabilizing solution X (n x n). A and Q are n x n, B is n x m, R is m x m.', usage: '[X] = dare(A[1:2,1:2], B[1:2,1:1], Q[1:2,1:2], R)' },
      { label: 'lyap (continuous Lyapunov solve)', snippet: '[$0X] = lyap(A, Q)', description: "Solve the continuous-time Lyapunov equation A X + X A' + Q = 0 for X (n x n). Used for stability analysis and gramians.", usage: '[X] = lyap(A[1:2,1:2], Q[1:2,1:2])' },
      { label: 'dlyap (discrete Lyapunov solve)', snippet: '[$0X] = dlyap(A, Q)', description: "Solve the discrete-time Lyapunov (Stein) equation A X A' - X + Q = 0 for X (n x n).", usage: '[X] = dlyap(A[1:2,1:2], Q[1:2,1:2])' },
      { label: 'rank (matrix rank)', snippet: '[r] = rank(M)', description: 'Compute the numerical rank of matrix M using Singular Value Decomposition (SVD) tolerance comparison.', usage: '[r] = rank(M[1:3,1:3])' },
      { label: 'ss2ss (state similarity transform)', snippet: '[$0An, Bn, Cn, Dn] = ss2ss(A, B, C, D, P)', description: 'Apply a similarity transformation matrix P to a state-space system (A, B, C, D) such that x = P * z, yielding transformed matrices An, Bn, Cn, Dn.', usage: '[An, Bn, Cn, Dn] = ss2ss(A[1:3,1:3], B[1:3], C[1:3], D, P[1:3,1:3])' },
      { label: 'stepinfo (transient metrics)', snippet: '[Tr, Tp, Ts, OS] = stepinfo(t, y)', description: 'Extract transient response metrics (Rise Time Tr, Peak Time Tp, Settling Time Ts, and Percent Overshoot OS) from step response outputs y(t) at time points t.', usage: '[Tr, Tp, Ts, OS] = stepinfo(t[1:100], y[1:100])' },
      { label: 'pade (time delay approximation)', snippet: '[$0num_delay, den_delay] = pade(Td, order)', description: 'Compute the numerator and denominator polynomial coefficients for a Padé approximation of time delay Td of a given order.', usage: '[num_delay, den_delay] = pade(Td, order)' },
      { label: 'rlocus (root locus trajectories)', snippet: '[$0K, cpr, cpi] = rlocus(num, den)', description: 'Compute closed-loop pole trajectories (real parts cpr, imaginary parts cpi) by sweeping M gain steps K for a transfer function or state space system.', usage: '[K, cpr, cpi] = rlocus(num[1:3], den[1:3])' },
      { label: 'routh (Routh-Hurwitz stability)', snippet: '[nRHP, stable] = routh(den)', description: 'Routh-Hurwitz test on a characteristic polynomial den (descending powers). Returns nRHP, the number of right-half-plane poles (sign changes in the first column), and stable (1 if nRHP=0, else 0). Handles the zero-in-first-column (epsilon) and row-of-zeros (auxiliary-polynomial) special cases.', usage: '[nRHP, stable] = routh(den[1:4])' },
      { label: 'c2d (continuous to discrete)', snippet: "[numz, denz] = c2d(num, den, Ts, '$0')", description: "Discretize a continuous transfer function num/den at sample time Ts. Method is a quoted 'tustin' (bilinear, default) or 'zoh' (zero-order hold). num and den must be the same length; outputs numz/denz are that length in descending powers of z, normalized to a monic denominator.", usage: "[numz, denz] = c2d(num[1:2], den[1:2], Ts, 'tustin')" },
      { label: 'd2c (discrete to continuous)', snippet: "[num, den] = d2c(numz, denz, Ts, 'tustin')", description: "Convert a discrete transfer function numz/denz back to continuous time at sample time Ts using the inverse Tustin (bilinear) transform. Outputs num/den in descending powers of s, normalized to a monic denominator.", usage: "[num, den] = d2c(numz[1:2], denz[1:2], Ts, 'tustin')" },
      { label: 'residue (partial-fraction / inverse Laplace)', snippet: '[$0r_r, r_i, p_r, p_i, k] = residue(num, den)', description: 'Partial-fraction (Heaviside) expansion of num/den: residues (r_r, r_i), the matching poles (p_r, p_i), and the scalar direct term k, so that num/den = sum r_i/(s - p_i)^ord + k. The numeric inverse Laplace path — residues appear in the Solution window. Add a 6th output ord ([r_r, r_i, p_r, p_i, ord, k] = residue(num, den)) to handle repeated poles, where ord is the power of each (s-p) term.', usage: '[r_r, r_i, p_r, p_i, k] = residue(num[1:1], den[1:3])' },
      { label: 'nichols (Nichols chart data)', snippet: '[$0mag, phase] = nichols(num, den, omega)', description: 'Open-loop magnitude (dB) and unwrapped phase (deg) at frequencies omega, for a transfer function or state space system. Plot mag vs phase (xy kind) to form a Nichols chart.', usage: '[mag, phase] = nichols(num[1:2], den[1:2], omega[1:50])' },
      { label: 'errorconst (steady-state error constants)', snippet: '[Kp, Kv, Ka] = errorconst(num, den)', description: 'Static error constants for an open-loop G(s)=num/den (in lowest terms): position Kp = lim G, velocity Kv = lim s*G, acceleration Ka = lim s^2*G as s->0. Constants that are infinite for the system type are returned as Infinity.', usage: '[Kp, Kv, Ka] = errorconst(num[1:3], den[1:3])' },
      { label: 'mason (signal-flow graph gain)', snippet: '[T] = mason(G, source, sink)', description: "Overall transmittance of a scalar signal-flow graph by Mason's gain formula. G is a square node-gain matrix (G[i,j] = branch gain from node i to node j, 0 = no branch); source and sink are 1-based node numbers.", usage: '[T] = mason(G[1:4,1:4], 1, 4)' },
    ],
  },
]
