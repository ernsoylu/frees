...
\begin{aligned}
out.p &= in.p - dp \\
tevap &= \text{T\_sat}\left(\mathrm{fluid}, =in.p\right) \\
q &= frac\cdot ua\cdot \left(wall.t - tevap\right) \\
out.h &= \text{Enthalpy}\left(\mathrm{fluid}, =out.p, p=tevap + sh\right) \\
in.mdot &= \frac{q}{out.h - in.h} \\
out.mdot &= in.mdot \\
wall.qdot &= q
\end{aligned}
$$
