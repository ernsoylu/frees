...
tau_{e} &= u.sig \\
shaft.tau &= -tau_{e} \\
pm &= tau_{e}\cdot shaft.w \\
eff &= \text{eff\$}\left(shaft.w, tau_{e}\right) \\
s &= 0.5\,\left(1 + \tanh\left(\frac{pm}{epsp}\right)\right) \\
pe &= \frac{s\cdot pm}{eff} + \left(1 - s\right)\cdot pm\cdot eff \\
\left(p.v - n.v\right)\cdot p.i &= pe \\
p.i + n.i &= 0 \\
q &= pe - pm \\
heat.qdot &= -q
\end{aligned}
