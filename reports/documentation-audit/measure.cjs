...
function csv(file,rows) {const keys=Object.keys(rows[0]);fs.writeFileSync(path.join(out,file),[keys,...rows.map(r=>keys.map(k=>r[k]??''))].map(r=>r.map(x=>'"'+String(x).replaceAll('"','""')+'"').join(',')).join('\n')+'\n');}
