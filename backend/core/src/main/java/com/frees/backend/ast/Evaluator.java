...
public final class Evaluator {
    ...
    private static double evalCall(Expr.Call c, Map<String, Double> values, Map<String, ProcDef> defs) {
        ...
        // Synthetic eigendecomposition call: eigen$val$<k>$<n>, eigen$re$<k>$<n>,
        // eigen$im$<k>$<n> or eigen$vec$<i>$<k>$<n>, with the n*n matrix entries
        // (row-major) as arguments.
        ...
        // Synthetic determinant call: det$<n> with the n*n matrix entries
        // (row-major) as arguments. Evaluated by LU (O(n^3)) because the
        // closed-form cofactor expansion is O(n!) — see EquationParser.
        if (c.function().startsWith("det$")) {
            return evalDeterminant(c, values, defs);
        }
    ...
    private static final String DTABLE1 = "dtable1";
    private static final String DIFFERENTIATE1 = "differentiate1";
    ...
    /**
     * Evaluates a synthetic eigen$val$<k>$<n>, eigen$re$<k>$<n>, eigen$im$<k>$<n>
     * or eigen$vec$<i>$<k>$<n> call. Eigenpairs are sorted ascending by real part,
     * then imaginary part; eigenvectors are unit-normalized with their
     * largest-magnitude component made positive so results are deterministic.
     * val/vec are the real-spectrum forms and reject complex eigenvalues; re/im
     * (the two-output Eigenvalues form) carry a complex spectrum as its parts.
     */
    private static double evalEigen(Expr.Call c, Map<String, Double> values, Map<String, ProcDef> defs) {
        ...
        String kind = parts[1];
        boolean wantVector = kind.equals("vec");
        ...
        boolean complexAllowed = kind.equals("re") || kind.equals("im");
        if (!complexAllowed && ed.hasComplexEigenvalues()) {
            throw new IllegalStateException(
                    "Matrix has complex eigenvalues; this form supports real spectra only "
                    + "(symmetric matrices always qualify). Use the two-output form "
        ...
        java.util.Arrays.sort(order, Comparator
                .comparingDouble((Integer i) -> ed.getRealEigenvalue(i))
                .thenComparingDouble(ed::getImagEigenvalue));
        int k = Integer.parseInt(parts[2 + (wantVector ? 1 : 0)]);
        if (kind.equals("im")) {
            return ed.getImagEigenvalue(order[k]);
        }
        if (!wantVector) {
    ...
    /** Evaluates a synthetic det$<n> call: LU determinant of the n x n row-major
     *  matrix entries (0 for a singular matrix, per LU convention). */
    private static double evalDeterminant(Expr.Call c, Map<String, Double> values, Map<String, ProcDef> defs) {
        int n = Integer.parseInt(c.function().substring("det$".length()));
        double[][] m = readMatrix(c.args(), 0, n, n, values, defs);
        return new LUDecomposition(new Array2DRowRealMatrix(m, false)).getDeterminant();
    }
