...
/**
 * The CLOSED vapor-compression loop solved by integrate-to-steady, the way the reference simulator
 * does it (research corpus: the two-phase library/the thermal-hydraulic library capacitive volumes — "mass + energy
 ...
 * short. the reference simulator's fix (and this one): the capacitive volume's STATES are
...
class ChargeCycleTest {
  ...
  private String model() {
    ...
    return ("""
      // reference-style two-phase capacitive volume: STATES are (P, h).
  ...
  @Disabled("FRONTIER (frees connect/assembler, reference-grounded): the CORRECT "
      + "formulation is now in place -- reference-style two-phase capacitive volumes "
      + "with (P,h) STATES (research corpus the two-phase library: 'mass+energy conservation with "
      ...
      + "R-C-R chains solve; the closed loop has none). the reference simulator avoids this with "
