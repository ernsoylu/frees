...
// industry-standard-style destructuring call of a multi-output FUNCTION or CALL intrinsic:
//   [q, w]        = split(x)
...
// industry-standard-style element-wise operators. Two chars starting with '.', so they
...
// A trailing '$' marks a string variable (a reference solver convention): R$ = 'R134a'.
// A trailing '#' marks a built-in constant (a reference solver convention): R#, g#, sigma#.
IDENT
    : [a-zA-Z] [a-zA-Z0-9_]* ('$' | '#')?
    ;
