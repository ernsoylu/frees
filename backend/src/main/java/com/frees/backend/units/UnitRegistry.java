...
/**
 * Engineering unit table and a reference solver unit-expression parser.
 *
 * a reference solver rules honored here:
...
public final class UnitRegistry {
    ...
    static {
        ...
        // Temperature (multiplicative scale only; Convert is multiplicative in a reference solver)
    ...
    /** Parses an a reference solver unit expression like "kJ/kg-K", "m^3", "Btu/hr-ft^2-R" or "-". */
    ...
    private static Quantity parseProduct(String part, String full) {
        ...
        // Dash, star, and whitespace mean multiplication in a reference solver units. (Dot is
        // omitted so decimal exponents like m^1.5 survive; negative exponents
        // are unsupported — use the denominator instead, as a reference solver convention.)
    ...
    /** a reference solver Convert('From', 'To'): the multiplicative factor between two unit expressions. */
