...
/**
 * Orchestrates the full a reference solver solve pipeline:
...
public class EquationSystemSolver {
     ...
     * of their first appearance, which a reference solver uses for display.
     */
    ...
    /**
     * a reference solver Check/Format: verifies syntax and reports equation/variable counts,
    ...
    /**
     * a reference solver equation-based Integral: each F = Integral(f, t, a, b) drives t
    ...
    private Result solveWithIntegrals(EquationParser.ParseResult parsed,
        ...
        for (IntegralSolver.IntegralEquation ie : integrals) {
            if (!ie.constantLimits()) {
                ...
                // upper limit, as in a reference solver.
    ...
    /**
     * a reference solver Check Units: dimensional consistency warnings for the given source
    ...
    /**
     * a reference solver assigns a variable its units when it is set from an annotated
    ...
    private Result buildResult(List<Equation> equations, List<Block> blocks,
        ...
        for (Map<String, Double> values : solutionMaps) {
            ...
            // (a reference solver Check/Format behavior); lookups stay case-insensitive.
