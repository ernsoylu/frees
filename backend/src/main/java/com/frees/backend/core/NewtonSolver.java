 ...
 *
 * Stopping follows the a reference solver Stop Criteria: iterations stop successfully when
...
public class NewtonSolver {
        ...
        for (int iteration = 0; iteration < settings.maxIterations(); iteration++) {
            ...
            for (int halving = 0; halving < MAX_HALVINGS; halving++) {
                for (int i = 0; i < n; i++) {
                    ...
                    // (a reference solver: bounds constrain the solver's search space).
            ...
            // a reference solver stop criterion: change in variables below threshold.
    ...
    /** a reference solver relative residual: |lhs - rhs| / |lhs|, guarding against |lhs| ~ 0. */
