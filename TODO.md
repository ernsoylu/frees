...
**Goal:** Bring a matrix language-Control-Toolbox-style workflows (TF/SS/ZPK models, interconnection,
...
- **Multi-output functions go through the existing `CALL Name(inputs : outputs)` path.**
  ...
  industry-standard-style bracket destructuring (`[mag, phase] = bode(...)`) is a later ergonomics layer
