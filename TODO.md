...
Goal: a feature-rich ODE engine comparable to block-diagram tools/a matrix language's solver family
...
* Header options mirror a matrix language `odeset` + block-diagram tools solver names: `method`, `t = t0 .. tf`,
...
**Why a block, not bare `der()` equations (an acausal language-style):** isolating the dynamic system
...
* **Fixed-step explicit** (block-diagram tools fixed-step): `ode1` Euler, `ode2` Heun, `ode3` (RK3 /
...
* **Variable-step explicit**: `ode45` Dormand–Prince 5(4) [default], `ode23` Bogacki–Shampine
  ...
  (rtol/atol), industry-standard-style. Dense output for sampling at requested `points` (DOPRI 4th-order
  interpolant).
...
* [x] Universal constant library — a reference solver `#`-suffix constants (`pi#`, `e#`, `R#`, `g#`, `Na#`, `k#`, `h#`, `c#`, `sigma#`, `Gc#`, `qe#`) via new `ConstantsRegistry`; grammar `IDENT` accepts `#`; substituted at parse time with SI units. `pi` → `pi#` (no backward compatibility).
...
* Change Solutions window matrix-language like Variable window.
