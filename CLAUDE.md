...
**frEES** is a web-based, 100% functional clone of the the reference equation solver (a reference solver). It uses a Java Spring Boot backend for high-performance symbolic compiling and numerical solving, and a React 19/TypeScript frontend for a multi-window dashboard interface.
...
## Core a reference solver Principles

- a reference solver is an **equation solver**, not a sequential programming language. Equations can be entered in any order.
...
- **All calculations run in SI** (frEES decision, beyond a reference solver): unit-annotated constants are converted to SI at parse time, and computed variables get dimensionally derived SI units. Unit warnings never block solving.
...
**Check-before-Solve (a reference solver Check/Format):** `POST /api/check` verifies syntax and structural solvability (zero degrees of freedom + complete equation↔variable matching) without solving. The frontend gates the Solve button on a successful check; any edit invalidates it.
