...
- **Client-IP trust is explicit, never assumed.** `X-Forwarded-For` is append-only, so only its RIGHT-hand end is trustworthy; `RequestGuardFilter.clientIp()` reads `X-Real-IP` (which nginx *replaces*) or the LAST `X-Forwarded-For` element — never the first, which is whatever the caller typed. Reading the first element made every rate limit a no-op: a fresh header per request bought a fresh counter per request. nginx recovers the true client behind an edge proxy via `real_ip`, but **only** for the hops listed in `TRUSTED_PROXY_CIDRS` (generated into `/etc/nginx/real-ip.conf` by `docker-entrypoint.d/10-real-ip.sh`), which is **empty by default**. Do not "helpfully" default it to the private ranges: with a docker-published port nginx's peer is always the bridge gateway, so trusting those would let any internet client forge its own address and re-open the bypass. Set it per deployment to the edge's actual source range (Railway: `fd00::/8`; Pangolin: the tunnel's range).
