...
- **Connection** — two surface syntaxes, same expansion: **shared stream name** (terse, for 2-port series chains) and **`connect(a, b, …)`** (an acausal language-style, native branching + union-find loop closure). A node emits across-equalities + a single `Σflow=0`.
