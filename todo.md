...
>
> **BUILD STATUS — Phase L COMPLETE.** **(1) Library reorganization:** the monolithic `ComponentLibrary.SOURCE` string is split into per-domain `resources/components/*.frees` files (`fluid`/`liquid`/`twophase`/`heat`/`electrical`/`mechanical`/`powertrain`/`control`/`moistair`/`pneumatic`/`hydraulic`), concatenated in fixed order at startup into the same parse-once immutable `BUILTINS` registry — parses identically (existing component suite green). **(2) Liquid domain (THH-equivalent):** `liquid.frees` adds `LiquidSource`/`LiquidSink`/`LiquidPump`/`LiquidPipe`/`LiquidColdPlate`/`LiquidVolume`/`LiquidOrifice` (with an optional THH-style `cavitating` VARIANT) on connector type `domain$ = liquid` — the same `(P,ṁ,h)` bond as `fluid` but a distinct connector type. `checkFluidConnectorType` rejects liquid↔fluid/gas/oil/twophase/moistair (string-compare; error message updated to list all six families + "couple them only through a heat exchanger"). *Validated:* a battery cold-plate water loop solves; a liquid pump shows the small incompressible `v·ΔP/η` work; liquid↔fluid and liquid↔hydraulic connects are rejected; the split library loads all 97 built-ins. 
>
> **BUILD STATUS — Phase T0 COMPLETE.** **(1) Correlations (3-site fn pattern, `props/TwoPhase`):** `void_homogeneous`/`void_zivi`/`void_rouhani` (Rouhani–Axelsson drift-flux default), `friedel_phi2` (two-phase frictional multiplier on the liquid-only drop), `momentum_flux` (separated-flow, accel-ΔP = out−in) — all SI, dome-clamped, unit-tested vs hand-calc. **(2) twophase domain (`twophase.frees`, `domain$ = twophase`):** `TwoPhaseSource` (by P,x), `TwoPhaseSink`, `TwoPhaseSensor` (x/α/SH derived **locally** from (P,h): `x=(h−hf)/(hg−hf)`, α via Zivi, SH=T−Tsat), `TwoPhaseFlowRes` (Friedel resistive line). Slimmed `(P,ṁ,h)` connector, no riders (§2.2). **(3) `never C-C` index-1 rule (`ComponentExpander`):** a connect-only union-find gives true node granularity; a node carrying two capacitive volumes (`der(port.P)`) is a hard `ParseException` directing to the C-R-C rule. *Validated:* an R134a `Source→FlowRes→Sensor→Sink` chain solves — quality/void compute locally, the line drops pressure and **T_sat glides down** with it; C-C rejected, C-R-C accepted; 101 built-ins; full suite green (974 tests). 
>
> **BUILD STATUS — Phase T1 COMPLETE.** Lumped vapor-compression components added to `twophase.frees` (`domain$ = twophase`): **`TwoPhaseCompressor`** (real-gas isentropic h(P_out,s_in) + η), **`TwoPhaseEvaporator`** (unified boiler≡evaporator: two-phase inlet → superheated outlet at a set exit superheat, **non-isobaric** out.P=in.P−dP, Q output), **`TwoPhaseCondenser`** (superheated → subcooled-liquid at a set subcooling, non-isobaric), **`TwoPhaseExpansionValve`** (isenthalpic Cv orifice on the upstream density, flashing-valid, odd-symmetric). Liquid pump is Phase L's `LiquidPump`. *Validated:* an R134a evaporator→compressor→condenser→valve cycle solves with refrigerant ΔP — the compressor **suction pressure falls below the evaporating pressure**, COP is a plausible 2–8, and **COP drops vs the dP=0 isobaric baseline** (where suction == evaporating). 105 built-ins; full suite green. 
>
> **BUILD STATUS — Phase T2 COMPLETE.** Charge inventory on `twophase.frees`: mass is `M = V·ρ̄` with the **void-weighted** density `ρ̄ = α·ρ_g + (1−α)·ρ_l` (α from Zivi slip, *not* CoolProp's no-slip (P,h) density — the mechanism that makes 1.5 kg ≠ 0.75 kg). Components: **`TwoPhaseInventory`** (pass-through charge accounting), **`TwoPhaseReceiver`** (saturated vessel, solved liquid level `LL`, saturated-liquid outlet, charge buffer), **`TwoPhaseCondenserUA`** (UA-to-ambient so the **condensing pressure is free** and set by the charge balance — the high-side tear of §4.6), **`TwoPhaseEnthalpySource`** (fixes ṁ,h, leaves P free). Pinning `inst.m` (the charge) releases the DOF. *Validated:* on the UA condenser, raising charge raises **condensing pressure, subcooling, and duty**; the receiver absorbs charge into its **liquid level** (stays in (0,1)) while always delivering saturated liquid (buffered); the inventory density is void-weighted (> homogeneous), so charge depends on the void model. 109 built-ins; full suite green. 
>
> **BUILD STATUS — Phase T3 core COMPLETE (moving-boundary assembly is the remaining increment).** Shipped & green: **(1) Nu correlations (3-site, `props/ConvectiveHeat`):** `nu_dittus_boelter`, `nu_gnielinski` (single-phase), `chen_f`/`chen_s` (Chen flow-boiling factors), `nu_shah` + `nu_cavallini_zecchin` (condensing), and **`zone_ramp(L,ε)=tanh(L/ε)`** (the §4.8 zone-collapse HT/storage ramp). **(2) C/R primitives + boundaries (`twophase.frees`):** `TwoPhaseVolume` (C — pressure-state `der(in.P)`, compliance, void-weighted charge `m`), with `TwoPhasePressureSource`/`TwoPhasePressureSink` (free-ṁ reservoirs) and `TwoPhaseCap`; `TwoPhaseFlowRes` (R) from T0. **(3) Few-cell distributed behaviour:** a multi-cell R-chain coil shows **temperature glide** along it (steady); a **C-R-C transient runs on the SUNDIALS-IDA path (`method = ida`)** — the volume pressure relaxes between two reservoirs and its void-weighted **charge migrates**, no NaNs. 113 built-ins; full suite green.
> **Moving-boundary HX (T3 final increment) — DONE.** `MovingBoundaryEvaporator`/`MovingBoundaryCondenser` (`twophase.frees`): geometry-resolved 2-zone HX with a **wall heat port**, the refrigerant-side zone boundary a solved length that **moves** with conditions. A shrinking zone is the §4.8 way — length smoothly floored at 0 (C¹ `½(x+√(x²+ε²))`) and heat transfer `zone_ramp`-gated, so a collapsing zone fades to an inert passthrough (`out.h →` saturation enthalpy), never a singular UA. Per-zone effective `U` params (compute from the shipped `chen_*`/`nu_*` for a regime-resolved design). Plus a general `(P,h)` boundary `TwoPhaseSourcePH`. *Validated:* the evaporator resolves both zones with superheat (steady); the superheat zone **collapses smoothly** (floored, no NaN) at low wall ΔT; the condenser resolves condense+subcool with subcooling; and a **transient warm-up on `method = ida`** (wall thermal mass) **births the superheat zone** as the wall warms, crossing the ramp region without a NaN. 116 built-ins; full suite green. **Phase T3 COMPLETE.**
>
> **BUILD STATUS — Phase AC COMPLETE — the full S1→L→T0→T1→T2→T3→AC sequence is now done.** Application package in its own `ac.frees` namespace, cross-domain bridges built **hierarchically** (refrigerant-side moving-boundary half + a secondary-side half, separate single-domain leaves coupled through a shared **wall heat node** — never a direct cross-domain connect, and this also gives per-side connector types for free). Shipped: **`Chiller`** (twophase↔liquid: `MovingBoundaryEvaporator` + `LiquidWallHX`) — the **EV chiller**; **`AirCoil`** (twophase↔moistair: evaporator + a wet-coil `MoistAirWallHX` doing **sensible + latent** dehumidification with a smooth-clamped saturated outlet); **`EXV`** (signal-controlled `u·CdA` orifice); **`TXV`** (quasi-static superheat-target opening governed by **two first-order states** — bulb thermal lag + valve-opening lag, §4.4); and a **volumetric/RPM `VARIANT`** on `TwoPhaseCompressor` (`ṁ = η_v·V_d·(rpm/60)·ρ_suction`). *Validated:* the chiller chills a water coolant loop (wall-node Q balances) and **delivers more cooling at higher refrigerant flow (RPM proxy)**; the volumetric compressor's flow scales linearly with RPM (1000→2500); the TXV's two lags relax to the superheat-target opening on `method=ida`; the AirCoil cools and dehumidifies (latent duty > 0). **122 built-ins; full backend suite green.**
>
> **PLAN COMPLETE.** All ratified phases delivered on branch `feat/twophase-refrigeration-s1`.
>
> **BUILD STATUS — three deferred fidelity rungs now also delivered (additive, full suite green):** **(S2)** **Jacobian coloring** — `DaeJacobian.colorColumns`/`denseColored` do a distance-1 column coloring so the sparse FD Jacobian costs `#colors` residual evals instead of `n` (≈ bandwidth for a banded C-R-C); wired into `IdaDaeSolver.fillSparseJacobian`, guard-tested colored==dense. (The KLU symbolic factorization is already reused across same-pattern setups, so the residual-eval count was the real cost.) **(MB regime)** the `MovingBoundaryEvaporator`/`Condenser` now use a **C¹ smooth-min** on the zone length — when the tube is long enough it reduces *exactly* to the prior complete-evaporation/condensation behavior, and when it is undersized the two-phase zone fills the tube and the **outlet correctly stays two-phase (x<1)** instead of capping at saturation (tested). **(Zeotropic glide)** an **opt-in composition rider `z`** — `acrossMembersForNode` adds `z` only for twophase nodes whose streams carry a `z` member (pure/azeotropic fluids carry none → no-op, the connector stays `(P,ṁ,h)`); `BlendSource`/`BlendSensor`/`BlendSink`/`BlendMixer` transport and flow-weight `z` (like moist-air `w`) and report the bubble→dew **temperature glide** (R407C ≈ several K; pure R134a ≈ 0). 126 built-ins. Remaining (genuinely larger / niche): variable-composition mixture *property* lookups `z→T_sat` (needs a mixture-property-function family — the static-`fluid$` baking blocks it), ECMS/event-heavy stop-start, full multi-species vector.
...
**In scope:** lumped + few-cell moving-boundary two-phase, charge-aware, correlation-based, on the acausal solver (+ `DYNAMIC`/SUNDIALS-IDA). **Out of scope (Tier C / different product):** full many-cell 1-D finite-volume with per-cell momentum/acoustic dynamics (the reference simulator `libcfd1d` territory), geometry→UA from plate/fin counts, and stop-start event-heavy distributed dynamics at scale. frEES targets the **transparent, design-point-to-light-transient** band; a 20-cell FV HX is no longer hand-readable, which trades away frEES's core differentiator.
...
*Appendix — the reference simulator mapping (board-confirmed reference):* frEES `liquid` ↔ `the thermal-hydraulic library`; frEES `twophase` ↔ `the two-phase library` (the reference simulator's `hflow` port carries P, h, T, x, α, SH/SC — **frEES deliberately slims its connector to `(P, ṁ, h)` and derives x/α/SH/SC locally**, §2.2; C/R primitives kept); frEES AC package ↔ `libac` (compressor/evap/cond/TXV/receiver + `TPF-THH` chiller + `TPF-GMMA` air coil).
