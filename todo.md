...
| **2** | Multi-domain ports + variant selector | 🟡 **started** | `model$` selector ✅ + heat `(T,Q̇)` port & primitives ✅ **shipped**; remaining: electrical `(V,I)`, mechanical `(τ,ω)` ports + their primitive components (§16) |
...
- ~~**Component variant selector (`model$`)** — Phase 2 first increment~~ **SHIPPED** (finding 10); ~~**thermal `(T, Q̇)` port + domain-aware `connect`**~~ **SHIPPED** (finding 11): conduction/convection/radiation/thermal-source primitives, `ΣQ̇=0` Kirchhoff node, member-keyed domain classification (`ComponentThermalTest`). **Next Phase 2 increment:** the **electrical `(V, I)` port** — reuses the exact same pattern (across `V` equal, flow `ΣI=0` Kirchhoff), plus R/source primitives and a battery-ECM (R-int) stub; then the **mechanical `(τ, ω)` port**. (Thermal mass / capacitive volume / battery SOC are Phase 3 — they need the storage→`DYNAMIC` path.)
