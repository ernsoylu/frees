...
>
> **Strict connector-domain separation (added 2026-06-27).** Pneumatic and hydraulic reuse the fluid node *algebra* (`(P,ṁ,h)`) but are now **distinct connector types**, so they can't be cross-wired. A `connect` node must be a single domain or it's a hard error: (1) a general through-variable guard rejects mixing bond-graph domains (heat↔electrical, fluid↔mechanical, …); (2) a reserved `domain$` param (`fluid` default / `gas` / `oil`) separates the fluid-family triple. Cross-domain coupling is only via transducer components (separate same-domain ports). `ComponentDomainSeparationTest` covers it. *(This was a guardrail the original plan lacked — pneumatic/hydraulic/thermofluid all classified as `FLUID` and connected silently.)*
