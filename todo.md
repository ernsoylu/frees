...
> **Remaining T3 increment:** assemble the **3–5-zone switched moving-boundary** `Evaporator`/`Condenser` *component* (N zones always, min-length floor, `zone_ramp`-gated HT + storage, one zone-collapse `EVENT` per zone, mode-frozen `IDACalcIC` re-init). All building blocks now exist (`zone_ramp`, Chen/Shah Nu, the C primitive, IDA `IDARootFind`+consistent-init from S1, DYNAMIC `EVENT` support); the remaining work is authoring + tuning that one component. Next action: **Phase AC** (`Chiller` twophase↔liquid, `AirCoil` twophase↔moistair, `TXV` 4-quadrant map + bulb lags, `EXV`) — or finish the moving-boundary component first.
