# frEES Backend a numeric package Parity Improvements
...
* **a numeric package Control Functions Integration:**
...
## Completed — Tier 4 a numeric package Parity (backend)
...
* **`kalman` (deferred by design):** a numeric package's `kalman` is the online filter time/measurement update (iterative, system-returning) — doesn't fit frees' static-solve model. `lqe` covers the steady-state estimator gain observer design needs. Revisit only for a transient/online filter, which would pair with the DYNAMIC solver.
...
```typescript
{ value: 'spreadsheet', label: 'Spreadsheet', tip: 'Spreadsheet — spreadsheet-style data editor', icon: IconGrid4x4 },
```
...
- **Import:** Support pasting spreadsheet/CSV data into the spreadsheet (FortuneSheet handles this natively)
...
- [x] Clipboard paste from a spreadsheet tool works
