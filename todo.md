...
| **3** | Transient mode (storage → `DYNAMIC`) | 🟡 **started** | storage routing ✅ (`der(member)`/`init(member)` → `DYNAMIC`; `ThermalMass` integrates, §15.2.16); remaining: more storage components (volume, L/C, SOC), display-name ODE accessor, stiff integrator + events, steady/transient duality |
...
| **Battery thermal coupling** | **self-heat `Q̇=I²R₀`** ✅ (`BatteryThermal`, steady) → `+ I·T·dV_oc/dT` → thermal-mass dynamics | 🟡 | E+T ✅ / S (dynamics need Phase 3) |
