...
**Status:** Design complete; **Phase 0 done, Phase 1 mostly done** — flow-resistance functions; the full core component layer; standard library (13 components, **strict no-default parameters**); Rankine/Brayton/refrigeration cycles; the fan-duct operating point; branching (Splitter/Mixer/HX); and **derived-property member access** (`s.T`/`s.x`/…) with per-port stream→fluid inference are shipped and green. Only the frontend-coupled §6 state-circuit binding (cycle plots) remains in Phase 1; the §8.5 variable-scaling gap is now the top robustness blocker (see §15 for live status & findings). This document is the agreed reference for Phase 0–6 delivery. Revision **R1** incorporates the advisory review (see §0).
...
- **Phase R / §8.5–8.6 solver robustness** (findings 4, 5, 8) — the top blocker, and bigger than first thought (see finding 8): the real work is **bounded/range-aware FD Jacobian perturbation** (keep CoolProp arguments inside their valid box while probing) + **row/column equilibration** + **§8.7 homotopy**, not just initial guesses (which were tried and reverted). Until then, implicit-property boundaries need seeds.
