...
- ~~**Component variant selector (`model$`)** — Phase 2 first increment~~ **SHIPPED** (finding 10); ~~**thermal `(T, Q̇)` port**~~ **SHIPPED** (finding 11); ~~**electrical `(V, I)` port**~~ **SHIPPED** (finding 12); ~~**mechanical `(τ, ω)` port**~~ **SHIPPED** (finding 13): torque/speed source, rotational damper, ground, ideal gear TF (`ComponentMechanicalTest`). ~~**cross-domain transducer**~~ **SHIPPED** (finding 14); ~~**EV battery-thermal flagship steady slice**~~ **SHIPPED** (finding 15): `BatteryThermal` self-heat → cold plate → coolant solves the electrical+thermal flagship topology steady-state. **Phase 2 complete.** **Next: Phase 3 — storage → `DYNAMIC`** (integration design now mapped in the Phase 3 section). Smallest first slice: a lumped **thermal mass** (`C·der(T)=ΣQ̇`) driven by the shipped conduction/convection primitives and integrated by a `DYNAMIC` header, asserting it relaxes to the Phase-2 steady temperature — after clearing the one grammar blocker (dotted-member initial conditions, or a storage `INIT` param). The C/I storage auto-classification + `der(X)` emission unlocks the transient half of every domain at once: **thermal mass** (`C·dT/dt=ΣQ̇`), **capacitive fluid volume** (`dm/dt=Σṁ`), electrical **L/C**, and the **battery SOC + RC + self-heating** dynamics — i.e. the transient EV-flagship. This needs the storage→`DYNAMIC` path (and, per Phase R, a stiff integrator for the mixed time constants).
