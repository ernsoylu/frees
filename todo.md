...
| **3** | Transient mode (storage → `DYNAMIC`) | 🟡 **started** | storage routing ✅ (`der(member)`/`init(member)` → `DYNAMIC`; `ThermalMass` integrates) + display-name accessor ✅ (§15.2.16); remaining: more storage components (volume, L/C, SOC), stiff integrator + events, steady/transient duality |
