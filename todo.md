...
**Status:** Design complete; **Phase 0 done, Phase 1 mostly done** — flow-resistance functions; the full core component layer; standard library (13 components, **strict no-default parameters**); Rankine/Brayton/refrigeration cycles; the fan-duct operating point; branching (Splitter/Mixer/HX); and **derived-property member access** (`s.T`/`s.x`/…) with per-port stream→fluid inference are shipped and green. Only the frontend-coupled §6 state-circuit binding (cycle plots) remains in Phase 1; the §8.5 robustness gap is now **largely closed** — property-argument seeding (valid base point) and range-aware FD-Jacobian perturbation (no NaN-poisoned columns) are shipped, leaving **§8.7 two-phase-dome homotopy** as the remaining solver blocker (see §15 for live status & findings). This document is the agreed reference for Phase 0–6 delivery. Revision **R1** incorporates the advisory review (see §0).
...
This plan adds an **acausal, multi-domain, component-based system-modeling layer** on top of that foundation. A *component* is a reusable, parameterized template of acausal equations with typed **ports**; instantiating and connecting components **expands into scalar equations** that flow through the *existing* solver unchanged. The theoretical basis is a **pseudo bond graph** (the same formalism underlying Siemens the reference simulator and an acausal language.Fluid).
...
1. The `(P, ṁ)` pair is a **pseudo bond** (relaxes `e·f = power` while keeping junction algebra) — exactly the choice the reference simulator makes for thermo-hydraulic/thermal libraries.
...
| Axis | an acausal language | the reference simulator | GT-SUITE | **frEES (this plan)** |
|---|---|---|---|---|
...
9. **§8.5 resolved into three independent modes; two are now shipped, the third is §8.7 (corrects finding 8).** A controlled experiment ladder on single `Temperature(P,h)=T` inversions (supercritical 30 MPa = monotonic; subcritical 8 MPa = dome-crossing; with/without seed) separated three *distinct* failure modes that finding 8 had conflated:
   ...
   - **(c) Two-phase dome plateau — STILL OPEN (§8.7).** Crossing the saturation dome, `dT/dh≈0` (temperature is flat across the dome), so the inversion is non-monotonic and Newton stalls/overshoots even with a valid seed and a finite Jacobian. The subcritical 8 MPa liquid→superheated inversion is the canonical case (`PropertyArgumentSeedingTest.subcriticalInversionAcrossDomeNeedsHomotopy`, `@Disabled`). **Needs §8.7 homotopy / phase-event handling** (continuation in T or a quality-bracketed sub-solve), not a guess or Jacobian tweak. *Practical guidance unchanged:* prefer explicit-`Enthalpy` forward forms, or seed past the dome, until §8.7 lands.

### 15.3 Immediate next steps (revised ordering)
- **Phase R / §8.7 homotopy is now the remaining solver-robustness blocker** (finding 9c) — modes (a) invalid base point and (b) NaN-poisoned Jacobian are **shipped** (property-argument seeding + range-aware FD perturbation; `PropertyArgumentSeedingTest`). The open item is **two-phase dome-crossing inversions** (`dT/dh≈0`): add continuation/phase-event handling so a liquid-range start can climb to a superheated answer. Row/column equilibration (§8.5) remains a worthwhile general conditioning add but is no longer the gate. Until §8.7 lands, dome-crossing implicit boundaries still need a past-the-dome seed.
