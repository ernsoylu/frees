...
This plan adds an **acausal, multi-domain, component-based system-modeling layer** on top of that foundation. A *component* is a reusable, parameterized template of acausal equations with typed **ports**; instantiating and connecting components **expands into scalar equations** that flow through the *existing* solver unchanged. The theoretical basis is a **pseudo bond graph** (the formalism underlying an acausal language.Fluid and established commercial 1-D multi-domain system simulators).

**Thesis.** This turns frEES into a declarative **system/network modeler** — power cycles, refrigeration/HVAC, flow networks, EV thermal management — that occupies the 0-D lumped, multi-domain, steady-and-transient band, while remaining transparent (every component is visible, editable, unit-checked equations) and web-based. It is, in effect, *a reference solver + the acausal toolboxes/block-diagram tools plant-to-control workflow in one document*, achieved with a thin parser/expander layer plus a short list of constitutive functions — **not** a new solver.
...
2. **Energy conservation is enforced explicitly by component equations** (and a mixing junction's flow-weighted enthalpy balance), not implicitly by the junction. This is precisely why an acausal language.Fluid introduced the `stream` connector.
...
| Axis | an acausal language | Causal 1-D suite | GT-SUITE | **frEES (this plan)** |
|---|---|---|---|---|
...
- **`connect(a, b, …)`** (an acausal language-style, native branching): emits `across` equalities and the single `Σflow=0` for the node — so splitters fall out for free; only mixing (flow-weighted enthalpy) needs a component.
...
| **1** | Core component layer (fluid, steady) | ✅ **complete** | §6 cycle plots ✅ — `CyclePathResolver` now recognises component stream members (`s1.P`/`s1.h`, indexed by the stream's digits) and emits the cycle overlay the existing property-plot renderer draws (§15.2.29). |
| **R** | Solver robustness & diagnostics | ✅ **complete** | §8.5 column equilibration ✅ + stream-member seeding ✅; backtracking ✅; range-aware FD + two-phase bracketing ✅; stiff `ode23s` ✅; source-mapped diagnostics ✅; **Mermaid topology view ✅** (backend `TopologyGraph` emits the component-network flowchart; frontend `TopologyTab` renders it — §15.2.28). |
...
| **5** | Datasheet component wrappers | ✅ **complete** | **valve `Cv`** ✅ (squared-resistance `ṁ·\|ṁ\|=Cv²ρΔP`, smooth at ΔP=0); **coupled flow-resistance networks** ✅ and **datasheet pump-curve (TABLE) operating point** ✅ now converge — unblocked by the §8.5 stream-pressure seeding + the smooth constitutive form (§15.2.25). The digitize→TABLE→component→Newton pattern works for coupled cases, not just single components. |
| **6** | Domain breadth & flagship system | ✅ **complete (backend)** | EV battery-thermal flagship (steady+transient) · EV powertrain · **subsystem/hierarchy + param-passing** (recursive) · translational `(F,v)` port · **multi-zone HX** (2-cell subsystem) · **psychrometric coil** (CoolProp HumidAir) · PMSM · turbocharger · **TXV/expansion valve** · system uncertainty — ~46-component library. **Only residual: ECMS** (HEV *online energy-management* — an optimizer-coupled control strategy, not a physical component; the frees `Optimizer` exists but wiring it is a dedicated effort). Motor/compressor η-maps = the proven datasheet-TABLE pattern (finding 25). |
...
- **Per-component unit tests** cross-checked against advisory-corpus references (the practice already used for the shipped libraries: the standard literature for cycles/compressible flow; the standard compact-HX text / the standard literature for ε-NTU; an open-source chemistry library/GRI-Mech for thermochemistry; the standard literature Table A-28 for equilibrium Kp).
...
29. **§6 cycle plots for the COMPONENT layer — SHIPPED.** The Phase-1 residual: `CyclePathResolver.groupStateKnowns` now also matches **component stream members** (`COMPONENT_STREAM_STATE` = `s1.p`/`s2.h`/…), taking the stream name's trailing digits as the cycle state index and the dotted member as the property. So a component-built Rankine's streams (s1…s5) are flashed and connected into the same `cyclePath` overlay the **existing** property-plot renderer (`plots/figure.ts`) already draws — no frontend change needed (the rendering was already there). Test `api/CyclePathComponentTest` (CoolProp-gated): the component Rankine produces a non-empty cycle path. **Phase 1 is now fully complete.** (Optional follow-up: auto-group component streams into per-fluid `stateTableDefs` so the Fluid-States window lists them by circuit — a nicety; the cycle overlay, the core §6 deliverable, works.)

28. **Topology view (§14.2) — SHIPPED (first frontend feature, full vertical slice).** **Backend** `api/TopologyGraph.mermaid(source)` parses the document and emits a Mermaid `flowchart` of the COMPONENT network — instances as nodes (`name`/`type`), edges from both `connect(...)` endpoints and shared-name binding (deduped, undirected). Added a `topology` field to the solve response (`SolveController.SolveResponse` + populated from `cleanText`; `TopologyGraphTest`). **Frontend** `mermaid` dep added (code-split/lazy); `TopologyTab.tsx` renders the string via `mermaid.render` (dark theme); registered as a dockable **Topology** window in `App.tsx` (lazy import + panelTitles + panelContent) with a launcher in `WorkspaceChrome` (`IconSitemap`); `api.ts` carries `topology?: string`. Backend suite green; `npm run build` clean (mermaid lazy-loaded only when the window opens). The §14.2 read-only topology view is done.

27. **System uncertainty + source-mapped diagnostics + PMSM/turbo — SHIPPED.** (a) **System-level uncertainty** flows through component networks with no new machinery — a parameter tolerance (`UncertaintyOf(R_val)=0.5`) propagates to the network's cross-domain KPIs (`Q=20±2 W`, `T=301±0.1 K`); `core/SystemUncertaintyTest` (`uncertainties()` is flat-keyed, `variables()` display-keyed). (b) **Source-mapped diagnostics (§14.1):** the under/over-specified `Blocker` error now lists the involved variables in dotted display form (`$`→`.`), so the gap localises to a component/stream (`core/ComponentDiagnosticsTest`). (c) **`PMSM`** (surface-mount, id=0) and ideal-gas **`Turbocharger`** (power-balanced boost) — `core/ComponentMachineryTest`. Full suite green.

26. **Subsystem/hierarchy + translational port — SHIPPED (Phase 6 keystone + breadth).** **Hierarchy:** a `COMPONENT` body may now contain **sub-instances + internal `connect`s** (grammar `CompSubInst`/`CompSubConnect`; `ComponentDef.subInstances/subConnects`). The expander **flattens** subsystems into leaf instances before solving (`ComponentExpander.flattenInstance`): sub-instances are namespaced `outer.sub`, internal connects rewritten (outer port → its bound stream via a `portAlias` map; `sub.port` → namespaced; bare → namespaced), and it **recurses** (nested subsystems) with a cycle guard. `streamOf` updated to resolve dotted sub-instance names (last-dot split) and treat `$`-bearing refs as flat streams. Test `core/ComponentHierarchyTest`: two heaters wrapped as a `DoubleHeater` subsystem compose (Δh=1500); a `QuadHeater` of two `DoubleHeater`s flattens recursively (Δh=3000). **Translational `(F, v)` port** added to the domain machinery (across `vel`, flow `Σf=0`) with ForceSource/TransDamper/TransGround/TransMass; plus smooth `Diode` and `ContactResistance` (`ComponentBreadthTest`). Full suite green. *This makes the EV system composable as one reusable block — the last big architectural item.*

25. **Prerequisite closed → Phase 5 complete: stream seeding + smooth resistance.** The coupled-network gate from finding 24 is **fixed**, in two parts: (a) **stream-member physical seeding** (`EquationSystemSolver.seedStreamMemberGuesses`) — any flat member ending `$p`/`$t` still at the default guess is seeded to `10⁵`/`300`, so a coupled fluid network no longer starts with a negative ΔP (user/GUI guesses still win; only pressure/temperature touched — voltage/enthalpy deliberately excluded as ambiguous/reference-dependent). (b) **`Valve` rewritten to the squared-resistance form** `ṁ·|ṁ| = Cv²·ρ·ΔP` — smooth and reversible at `ΔP=0` (the raw `√(ΔP)` has an infinite derivative there, which seeding alone can't escape). Same answers, robust convergence. Now **two valves in series** (unknown mid-pressure) *and* a **datasheet pump-curve + valve operating point** (digitized `TABLE`) both converge from a cold start (`ComponentValveTest`, 3 tests). **Full suite green** — the seeding is neutral (initial guess only; every existing test unaffected). This was the genuine §8.5/§8.6 prerequisite, now done.

24. **Valve `Cv` (Phase 5) — first cut; the coupled-network stall it exposed is now fixed in finding 25.** `Valve(in, out)` flow-resistance; a single valve across a fixed drop solves exactly. (It surfaced that coupled `√(ΔP)` networks stall from the default guess — addressed by finding 25's seeding + squared form.)
...
18. **Phase 4 core — plant → control coupling SHIPPED (§8.3/§8.10).** The acausal toolboxes→block-diagram tools headline synergy: *build physical plant → auto-linearize → design controller → close the loop*, in one document.
