...
| **R** | Solver robustness & diagnostics | ✅ **complete (backend)** | §8.5 column equilibration ✅ + stream-member seeding ✅; backtracking ✅; range-aware FD + two-phase bracketing ✅; stiff `ode23s` ✅; **source-mapped diagnostics ✅** (DOF error lists involved stream members in dotted form, §15.2.27). The only residual is the **Mermaid topology *view*** — a frontend render (the graph data is available). |
...
| **6** | Domain breadth & flagship system | 🟢 **flagship + broad breadth + hierarchy** | **EV battery-thermal flagship** (steady + transient) ✅; **EV powertrain** ✅; **subsystem/hierarchy** ✅ (recursive); translational `(F,v)` port ✅; **psychrometric cooling coil** ✅ (CoolProp HumidAir, sensible+latent); ~41-component library. Remaining: TXV/multi-zone HX (loop-coupled CoolProp), turbocharger/PMSM (more physics), ECMS (advanced), system-uncertainty test — battery R-int/1RC, DC motor, Stribeck friction, clutch, planetary gear, engine, road-load, spring, PI controller, valve. Remaining (open-ended / CoolProp / larger features): motor-compressor **η-maps** (now unblocked — datasheet maps converge, finding 25; just need the user TABLE pattern), multi-zone HX + **psychrometric coil** (CoolProp HumidAir), **subsystem/hierarchy** (recursive component expansion — a genuine new feature), ECMS, system-level uncertainty |
