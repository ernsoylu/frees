...
- ~~**Component variant selector (`model$`)** — Phase 2 first increment~~ **SHIPPED** (finding 10); ~~**thermal `(T, Q̇)` port**~~ **SHIPPED** (finding 11); ~~**electrical `(V, I)` port**~~ **SHIPPED** (finding 12); ~~**mechanical `(τ, ω)` port**~~ **SHIPPED** (finding 13): torque/speed source, rotational damper, ground, ideal gear TF (`ComponentMechanicalTest`). ~~**cross-domain transducer**~~ **SHIPPED** (finding 14); ~~**EV battery-thermal flagship steady slice**~~ **SHIPPED** (finding 15); ~~**Phase 3 first slice — transient component storage**~~ **SHIPPED** (finding 16): `der(member)`/`init(member)` route a storage component network into the `DYNAMIC` block; `ThermalMass` integrates a first-order relaxation, steady limit = Phase-2 operating point (`ComponentTransientTest`). ~~display-name ODE accessor~~ ✅; ~~storage across all domains + transient EV flagship + steady/transient duality + stiff method~~ ✅ **SHIPPED** (finding 17) — **Phase 3 complete.** **Next: Phase 4 (plant→control coupling)** — numerically linearize a solved component network around its operating point into `(A,B,C,D)` (reuse the Newton Jacobian), hand off to the shipped control suite (`ss`/`tf`/`bode`/`lqr`/`pidtune`), close the loop via `DYNAMIC`. Alternatively **Phase 6 breadth** (motor/inverter η-map, friction/clutch + zero-crossing events, multi-zone HX, psychrometrics). The C/I storage auto-classification + `der(X)` emission unlocks the transient half of every domain at once: **thermal mass** (`C·dT/dt=ΣQ̇`), **capacitive fluid volume** (`dm/dt=Σṁ`), electrical **L/C**, and the **battery SOC + RC + self-heating** dynamics — i.e. the transient EV-flagship. This needs the storage→`DYNAMIC` path (and, per Phase R, a stiff integrator for the mixed time constants).
