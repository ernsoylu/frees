...
| **R** | Solver robustness & diagnostics | 🟢 **core complete** | §8.5 **column equilibration ✅ + stream-member physical seeding ✅** (`*$p`→10⁵, `*$t`→300); backtracking ✅; range-aware FD Jacobian + two-phase bracketing ✅; stiff `ode23s` ✅. Remaining (additive niceties): source-mapped diagnostics (§14.1), Mermaid topology (§14.2), structural high-index guard (§8.9) |
