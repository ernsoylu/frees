# frEES — a matrix language-Style Documentation Overhaul

## Goal

Replace the current help system — vague topic-guide prose plus hand-maintained one-line
function lists — with **industry-standard-style, per-function reference documentation**: granular,
...
and surfaces the exact errors a user can hit. North star: the a tool vendor function reference
...
reference (the standard literature, the standard literature, the standard compact-HX text) does.
...
- **Engineering categories** (Heat Transfer, Compressible, Two-Phase, Combustion, EOS, fluid) —
  ground every equation against the **research corpus** (`350fef25`) via `the research corpus ask`.
- **Control / Math / Matrix / Special / Stats** (≈130 fns, not in the notebook) — cite **standard
  references directly** (decided by user): the standard control texts (control), the standard matrix-computations reference (matrix/linear
  algebra), the standard special-functions reference / NIST DLMF (special functions), the standard literature/Devore (stats). Keep the
...
6. **Mathematical rigor with citations is mandatory, not optional.** Every page that
   ...
   source. **Ground every citation against the the research corpus "Frees" notebook**
   (id `350fef25-d19d-4542-a55d-7217276f077a`, 157 sources — the standard literature, the standard literature, the standard compact-HX text,
   the standard literature, the standard literature, the standard literature, the standard literature, a reference solver/Mastering-a reference solver, the reference simulator lib PDFs). The `the research corpus` CLI
   is authenticated:
   `the research corpus ask "<governing equation / method for X>" --notebook 350fef25 -s <srcid>`
   for a grounded, source-attributed lookup; use `the research corpus source list --notebook 350fef25`
...
references:                                       # grounded via the Frees notebook
  - "the standard matrix-computations reference, Matrix Computations, §3.4 (LU with partial pivoting)"
---
```
...
- **References** — full citations (textbook/standard + section), sourced from the Frees
  research corpus; renders as a footnote list, cross-linked from the formulas above.
...
- **Phase 0 — Inventory & scaffolding. IN PROGRESS.**
  ...
  - DONE — **Exemplar page** `reference/heat-transfer/hx_effectiveness.md` realizing the full
    standard: KaTeX ε-NTU relations grounded against the Frees notebook (the standard compact-HX text Eq. 2-13,
    the standard literature Eq. 2.46, the standard literature Eq. 10-27), bound to the verified example `hx-effectiveness-ntu`,
...
- **Phase 2 — Reference content (the bulk). IN PROGRESS (10/515 pages).**
  ...
  - DONE — Heat-Transfer seed (4): `hx_effectiveness`, `LMTD`, `fin_efficiency`, `hx_NTU` (the standard literature
    2.28/2.36, the standard literature 2-38/10-12, Özışık 3-41b, the standard compact-HX text Table 2.4); bound to
    `hx-effectiveness-ntu` / `ev-thermal-management`.
  - DONE — Compressible-flow nozzle set (6): `T0_T`, `P0_P`, `mach_A_Astar`, `M2_shock`,
    `P2_P1_shock`, `P02_P01_shock` (the standard literature Thermodynamics Eq. 17-18/19/26/38/39, Fig. 17-42,
  ...
  - DONE — Radiation view factors + Heisler (5): `viewfactor_disks`, `viewfactor_plates`,
    `viewfactor_perp`, `heisler_temp`, `heisler_q` (the standard literature Table 8-2 Items 1/3/5, Figs 8-12/14/16;
  ...
  - DONE — Control-systems set (9): `pole`, `zero`, `margin`, `bode`, `nyquist`, `step` (→
    ...
    the standard control texts per the standard-citation policy; exact poles/zero stated (s=−2±4.583j, z=−2).
  ...
  - DONE — Cubic-EOS set (5): `eos_z`, `eos_volume`, `eos_density`, `eos_enthalpy`, `eos_psat` (→
    ...
    to the standard literature Eq. 12-57 for the departure, but flagged the Z-cubic/fugacity as not in the notebook,
    so those cite the canonical primaries (Peng-Robinson 1976, the original SRK publication, a standard chemical-thermodynamics text).
  ...
  - DONE — Combustion seed (2): `AdiabaticFlameTemp`, `wiebe_rate` (notebook timed out → standard
    citations the standard literature/Turns, the standard literature). **31/475.**
  - DONE — HX correlations core (6): `ua_hx`, `htc_1phase` (Gnielinski), `htc_evap` (Shah boiling),
    ...
    `ev-thermal-management`; notebook-grounded (the standard literature Eq. 2.11/8.34/8.70, the standard literature 6-34, the standard literature &
    the standard literature 2.68) + primary papers. **37/475 documented (7.8%).**
  - DONE — HX geometry helpers (5): `hx_eta_surf` (the standard literature 11.3), `hx_aconv`, `hx_fin_len`,
    `hx_area_direct`, `hx_area_indirect` (the standard compact-HX text / Shah & Sekulić) → `ev-thermal-management`.
  ...
  - ENRICHMENT IN PROGRESS — upgrade baselines to rich pages (cited Mathematical Formulation +
    ...
    - DONE — Control (14): `tf`/`tf2ss`/`impulse`/`lsim`/`nichols`/`c2d`/`rlocus`/`routh`/`lqr`/
      `lqe`/`gram`/`balreal`/`pidtune`/`residue` (the standard control texts/Franklin/the standard literature-Ho/Moore/Åström).
    - DONE — Math/misc (14): `sqrt`/`sin`/`cos`/`exp`/`ln`/`min`/`max`/`diag` (light), `integral`,
      `interpolate2d`, `cond` (the standard matrix-computations reference), `if`, `StagnationTemp`/`StagnationPres` (the standard literature).
    ...
    - DONE — **all 399 remaining pages made content-complete from their authoritative source**
      ...
      standard reference (the standard special-functions reference, the standard matrix-computations reference, the standard control texts, the standard literature, the standard literature,
      the standard literature et al., US Std Atmosphere 1976, CoolProp, …). `tsc -b` green (20.6k-line catalog).
    ...
    - DONE — **Special Functions category fully rich (24/24)**: core 7 (gamma/loggamma/digamma/
      ...
      with KaTeX definitions/recurrences + the standard special-functions reference / NIST DLMF citations.
    - DONE — **Control Systems category fully rich** (22 more): place/acker/ctrb/obsv/lyap/dlyap/
      dare/dlqr/d2c/errorconst/stepinfo/parallel/pade/mason/ss/ss2ss/tf2zp/zp2tf (the standard control texts/
      Franklin/the standard literature) + Eigenvalues/Eigen/LUDecompose/EulerRotate (the standard matrix-computations reference/the standard literature).
    ...
    - **COMPLETE: 475/475 pages, 100% rich, 0 baselines.** The industry-standard-style per-symbol reference is
  ...
  research corpus** (`the research corpus ask … --notebook 350fef25 -s <srcid>`); bind to a verified
...
and the reference simulator-style charge-state closed refrigerant cycle all shipped green. Remaining open
