...
| **2** | Multi-domain ports + variant selector | 🟡 **started** | `model$` selector ✅ + heat `(T,Q̇)` ✅ + electrical `(V,I)` port & primitives ✅ **shipped**; remaining: mechanical `(τ,ω)` port + its primitive components (§16) |
...
- ~~**Component variant selector (`model$`)** — Phase 2 first increment~~ **SHIPPED** (finding 10); ~~**thermal `(T, Q̇)` port**~~ **SHIPPED** (finding 11); ~~**electrical `(V, I)` port**~~ **SHIPPED** (finding 12): R/source/ground + battery R-int ECM, 3-way `nodeDomain` classifier + shared `kirchhoffBalance` (`ComponentElectricalTest`). **Next Phase 2 increment:** the **mechanical `(τ, ω)` / `(F, v)` port** — same shape again (across `ω` equal, flow `Στ=0`), with inertia (algebraic at steady state), spring, damper, gear-ratio transducer, and a shaft source/sensor. That completes the four Phase-2 port domains; then **Phase 3** (storage→`DYNAMIC`) unlocks thermal mass, capacitive volume, L/C, and the battery SOC/RC/thermal dynamics. (Thermal mass / capacitive volume / battery SOC are Phase 3 — they need the storage→`DYNAMIC` path.)
