...
- **Bundle Budget Ceiling**: The compiled WebAssembly engine (`frees.wasm`) must strictly remain $\le 5,120\text{ KiB}$ raw (measured at ~3,888 KiB). Any PR exceeding this budget fails CI automatically. The lazy-chunk seam (Phase 4.2) is in place; CI warns when headroom reaches the 200 KiB trigger.
