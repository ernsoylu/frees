...
const AS = 'the standard literature, M. & the standard literature, I.A., *Handbook of Mathematical Functions*';
const GVL = 'the standard literature, G.H. & the standard literature, C.F., *Matrix Computations* (4th ed.)';
const THERMO_REF = 'the standard literature, Y.A., the standard literature, M.A. & Kanoğlu, M., *a standard thermodynamics text*, Ch. 17';
const ANDERSON = 'Anderson, J.D., *Modern Compressible Flow* (3rd ed.)';
const TWOPHASE_REF = 'the standard literature, J.G. & the standard literature, J.R., *a standard two-phase text* (3rd ed.)';
const HEATMASS_REF = 'the standard literature, F.P. et al., *Fundamentals of Heat and Mass Transfer*';
const FLUIDS_REF = 'the standard literature, *Fluid Mechanics* (8th ed.)';
const STATS_REF = 'the standard literature, D.C. & the standard literature, G.C., *a standard statistics text*';
const ISA76 = 'U.S. Standard Atmosphere, 1976 (NOAA/NASA/USAF)';
const COMBUST_REF = 'Turns, S.R., *An Introduction to Combustion* (3rd ed.)';
const NR = 'a standard numerics text, *a standard numerics text* (3rd ed.)';
...
const R = {
  ...
  // ── Compressible flow ──
  rho0_rho: { m: ['\\frac{\\rho_0}{\\rho} = \\left(1 + \\tfrac{k-1}{2}M^2\\right)^{1/(k-1)}'], r: [THERMO_REF + ', Eq. (17-20)'] },
  a_astar: { m: ['\\frac{A}{A^*} = \\frac{1}{M}\\left[\\frac{2}{k+1}\\left(1 + \\tfrac{k-1}{2}M^2\\right)\\right]^{(k+1)/[2(k-1)]}'], r: [THERMO_REF + ', Eq. (17-26)'] },
  t2_t1_shock: { m: ['\\frac{T_2}{T_1} = \\frac{\\big[1 + \\tfrac{k-1}{2}M_1^2\\big]\\big[\\tfrac{2k}{k-1}M_1^2 - 1\\big]}{M_1^2\\,(k+1)^2/[2(k-1)]}'], r: [THERMO_REF + ', Eq. (17-37)'] },
  rho2_rho1_shock: { m: ['\\frac{\\rho_2}{\\rho_1} = \\frac{(k+1)M_1^2}{2 + (k-1)M_1^2}'], r: [THERMO_REF + ', Eq. (17-36)'] },
  ...
  rayleigh_t0_t0star: { m: ['\\frac{T_0}{T_0^*} = \\frac{(k+1)M^2\\,[2 + (k-1)M^2]}{(1 + kM^2)^2}'], r: [THERMO_REF + ', Ch. 17 (Rayleigh)'] },
  rayleigh_t_tstar: { m: ['\\frac{T}{T^*} = \\left(\\frac{(k+1)M}{1 + kM^2}\\right)^2'], r: [THERMO_REF + ', Ch. 17 (Rayleigh)'] },
  rayleigh_p_pstar: { m: ['\\frac{P}{P^*} = \\frac{k+1}{1 + kM^2}'], r: [THERMO_REF + ', Ch. 17 (Rayleigh)'] },
  rayleigh_p0_p0star: { m: ['\\frac{P_0}{P_0^*} = \\frac{k+1}{1+kM^2}\\left[\\frac{2 + (k-1)M^2}{k+1}\\right]^{k/(k-1)}'], r: [THERMO_REF + ', Ch. 17 (Rayleigh)'] },
  fanno_t_tstar: { m: ['\\frac{T}{T^*} = \\frac{k+1}{2 + (k-1)M^2}'], r: [THERMO_REF + ', Ch. 17 (Fanno)'] },
  fanno_p_pstar: { m: ['\\frac{P}{P^*} = \\frac{1}{M}\\sqrt{\\frac{k+1}{2 + (k-1)M^2}}'], r: [THERMO_REF + ', Ch. 17 (Fanno)'] },
  fanno_p0_p0star: { m: ['\\frac{P_0}{P_0^*} = \\frac{1}{M}\\left[\\frac{2 + (k-1)M^2}{k+1}\\right]^{(k+1)/[2(k-1)]}'], r: [THERMO_REF + ', Ch. 17 (Fanno)'] },
  fanno_fld: { m: ['\\frac{4 f L^*}{D} = \\frac{1-M^2}{kM^2} + \\frac{k+1}{2k}\\ln\\frac{(k+1)M^2}{2 + (k-1)M^2}'], r: [THERMO_REF + ', Ch. 17 (Fanno)'] },

  // ── Flow networks ──
  reynolds: { m: ['Re = \\frac{\\rho V D}{\\mu}'], r: [FLUIDS_REF] },
  minor_loss: { m: ['\\Delta P = K\\,\\tfrac12\\rho V^2'], r: [FLUIDS_REF] },
  friction_factor: { m: ['\\frac{1}{\\sqrt{f}} = -2\\log_{10}\\!\\left(\\frac{\\varepsilon/D}{3.7} + \\frac{2.51}{Re\\sqrt{f}}\\right) \\quad\\text{(Colebrook; } f = 64/Re \\text{ laminar)}'], r: [FLUIDS_REF, 'Colebrook, C.F. (1939), J. Inst. Civ. Eng. 11:133'] },
  ...
  // ── Two-phase flow ──
  lm_phi2: { m: ['\\phi_l^2 = 1 + \\frac{C}{X} + \\frac{1}{X^2} \\quad\\text{(Chisholm)}'], r: [TWOPHASE_REF + ', Eq. (2.68)'] },
  lm_martinelli_tt: { m: ['X_{tt} = \\left(\\frac{1-x}{x}\\right)^{0.9}\\left(\\frac{\\rho_g}{\\rho_l}\\right)^{0.5}\\left(\\frac{\\mu_l}{\\mu_g}\\right)^{0.1}'], r: [TWOPHASE_REF + ', §2.4'] },
  void_homogeneous: { m: ['\\alpha = \\frac{1}{1 + \\frac{1-x}{x}\\frac{\\rho_g}{\\rho_l}} \\quad\\text{(no slip)}'], r: [TWOPHASE_REF + ', Ch. 2'] },
  ...
  friedel_phi2: { m: ['\\phi_{lo}^2 = E + \\frac{3.24\\,F H}{Fr^{0.045}We^{0.035}} \\quad\\text{(Friedel)}'], r: [TWOPHASE_REF + ', §2.5'] },
  momentum_flux: { m: ['\\left(\\frac{d P}{d z}\\right)_{\\text{acc}} = G^2\\frac{d}{dz}\\left[\\frac{x^2}{\\rho_g\\alpha} + \\frac{(1-x)^2}{\\rho_l(1-\\alpha)}\\right]'], r: [TWOPHASE_REF + ', Ch. 2'] },
  nu_dittus_boelter: { m: ['Nu = 0.023\\,Re^{0.8}\\,Pr^{n} \\quad (n = 0.4 \\text{ heating},\\ 0.3 \\text{ cooling})'], r: [HEATMASS_REF + ', Eq. (8.60)'] },
  nu_gnielinski: { m: ['Nu = \\frac{(f/8)(Re-1000)Pr}{1 + 12.7\\sqrt{f/8}\\,(Pr^{2/3}-1)}'], r: [HEATMASS_REF + ', Eq. (8.62)'] },
  ...
  dp_compact_core: { m: ['\\frac{\\Delta P}{P_1} = \\frac{G^2}{2\\rho_1 P_1}\\left[(1+\\sigma^2)\\!\\left(\\tfrac{\\rho_1}{\\rho_2}-1\\right) + f\\tfrac{A}{A_c}\\tfrac{\\rho_1}{\\rho_m}\\right] \\quad\\text{(the standard literature–London core)}'], r: ['the standard literature, W.M. & London, A.L., *Compact Heat Exchangers* (3rd ed.), Ch. 2'] },
  ...
  hx_dh: { m: ['D_h = \\frac{4\\,A_{\\text{flow}}\\,L}{A_{\\text{total}}}'], r: ['the standard literature, W.M. & London, A.L., *Compact Heat Exchangers* (3rd ed.), Ch. 2'] },
  hx_sigma: { m: ['\\sigma = \\frac{A_{\\text{flow}}}{A_{\\text{frontal}}} \\quad\\text{(free-flow / contraction ratio)}'], r: ['the standard literature, W.M. & London, A.L., *Compact Heat Exchangers* (3rd ed.), Ch. 2'] },
  ...
  nu_churchill_chu: { m: ['Nu = \\left\\{0.60 + \\frac{0.387\\,Ra^{1/6}}{[1 + (0.559/Pr)^{9/16}]^{8/27}}\\right\\}^2 \\quad\\text{(Churchill–Chu)}'], r: [HEATMASS_REF + ', Eq. (9.34)'] },
  nu_blend: { m: ['Nu = \\big(Nu_1^3 + Nu_2^3\\big)^{1/3} \\quad\\text{(free+forced cubic blend)}'], r: [HEATMASS_REF] },
  nu_hilpert: { m: ['Nu = C\\,Re^{m}\\,Pr^{1/3} \\quad\\text{(single cylinder, Hilpert)}'], r: [HEATMASS_REF + ', Eq. (7.52)'] },
  ...
  nu_traviss: { m: ['Nu = \\frac{Pr_l\\,Re_l^{0.9}\\,F(X_{tt})}{F_2} \\quad\\text{(Traviss condensation)}'], r: ['the original correlation publication
  j_fin: { m: ['j = St\\,Pr^{2/3} = C\\,Re^{m} \\quad\\text{(Colburn } j \\text{ for the fin surface)}'], r: ['the standard literature, W.M. & London, A.L., *Compact Heat Exchangers* (3rd ed.)'] },
  f_fin: { m: ['f = C_f\\,Re^{m_f} \\quad\\text{(Fanning friction for the fin surface)}'], r: ['the standard literature, W.M. & London, A.L., *Compact Heat Exchangers* (3rd ed.)'] },
  ...
  // ── Combustion ──
  adiabaticflametempeq: { m: ['H_{\\text{react}}(T_r) = H_{\\text{prod}}(T_{ad}) \\quad\\text{with equilibrium dissociation at } (T_{ad}, P)'], r: [COMBUST_REF + ', Ch. 2'] },
  eq_molefraction: { m: ['\\text{species mole fraction from chemical equilibrium } \\big(\\min G \\text{ at } T, P\\big)'], r: [COMBUST_REF + ', Ch. 2'] },
  ...
  wiebe: { m: ['x_b(\\theta) = 1 - \\exp\\!\\left[-a\\left(\\frac{\\theta-\\theta_0}{\\Delta\\theta}\\right)^{m+1}\\right]'], r: ['the standard literature, J.B., *a standard engine text*, Ch. 9'] },

  // ── EOS remainder ──
  eos_entropy: { m: ['s(T,P) = s^{\\text{ig}}(T,P) + (s - s^{\\text{ig}})_{T,P} \\quad\\text{(ideal-gas + EOS departure)}'], r: ['the standard literature, the standard literature, H.C. & the standard literature, *a standard chemical-thermodynamics text*, Ch. 6'] },
  eos_pressure: { m: ['P = \\frac{RT}{v-b} - \\frac{a\\,\\alpha(T)}{v(v+b) + b(v-b)} \\quad\\text{(PR; from } T, v)'], r: ['the original PR publication
};
