...
export const FUNCTION_CATEGORIES: FunctionCategory[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: '[a, b] = f(x) (multi-call / destructuring)', snippet: '[$0a, b] = fname(x)', description: 'industry-standard-style destructuring. Works for user multi-output FUNCTIONs AND every built-in multi-output CALL function (e.g. [A, B, C, D] = tf2ss(num, den)). Use ~ to discard a slot, or omit trailing outputs you do not need.', usage: '[small, large] = Order(8, 3)\n[A, B, C, D] = tf2ss(num, den)\n[~, ~, V] = svd(M)\n[A, B] = tf2ss(num, den)   { C, D dropped }' },
