...
export const FUNCTION_CATEGORIES: FunctionCategory[] = [
  {
    ...
    items: [
      ...
      { label: 'Fugacity', snippet: 'Fugacity($0, T=, P=)', description: 'Fugacity coefficient (dimensionless) of a fluid.', usage: 'f = Fugacity(CO2, T=300 [K], P=5 [MPa])' },
      { label: 'Enthalpy_fusion', snippet: 'Enthalpy_fusion($0)', description: 'Enthalpy of fusion (latent heat of melting) [J/kg].', usage: 'hf = Enthalpy_fusion(Water)   { ~333500 J/kg }' },
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'FreezingPt', snippet: 'FreezingPt($0)', description: 'Freezing point [K] of a material.', usage: 'Tf = FreezingPt(Water)   { 273.15 K }' },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      { label: 'Lookup', snippet: "Lookup('$0', row, col)", description: 'Return a single cell value from a TABLE by 1-based row and column indices.', usage: "v = Lookup('fanCurve', 3, 2)" },
      { label: 'LookupRow', snippet: "LookupRow('$0', col, val)", description: 'Row index where column col first equals val.', usage: "r = LookupRow('myTable', 1, 25)" },
      { label: 'Lookup$Row', snippet: "Lookup$Row('$0', col$, val$)", description: 'Row index where string column col$ first equals val$.', usage: "r = Lookup\\$Row('materials', 'Name', 'Steel')" },
      { label: 'NLookupRows', snippet: "NLookupRows('$0')", description: 'Number of data rows in a TABLE.', usage: "n = NLookupRows('fanCurve')" },
      { label: 'Interpolate', snippet: "Interpolate('$0', y_col, x_col, x_val)", description: 'Linear interpolation: find y_col value at x_val in x_col.', usage: "h = Interpolate('steam', 2, 1, 250)" },
      { label: 'Interpolate1', snippet: "Interpolate1('$0', y_col, x_col, x_val)", description: 'Cubic-spline interpolation on a TABLE column.', usage: "h = Interpolate1('steam', 2, 1, 250)" },
      { label: 'Interpolate2D', snippet: "Interpolate2D('$0', z_col, x_col, x_val, y_col, y_val)", description: 'Bi-linear (2D) interpolation over two independent columns.', usage: "z = Interpolate2D('grid', 3, 1, x, 2, y)" },
      { label: 'Differentiate', snippet: "Differentiate('$0', y_col, x_col, x_val)", description: 'Numerical derivative dy/dx at x_val from a TABLE.', usage: "dhdx = Differentiate('steam', 2, 1, 250)" },
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'FUNCTION (multi-output)', snippet: 'FUNCTION [$0out1, out2] = fname(x)\n  out1 := \n  out2 := \nEND\n', description: 'industry-standard-style function returning several outputs. Assign each output by name with := in the body, then call it with [a, b] = fname(x).', usage: 'FUNCTION [q, r] = DivMod(a, b) … q := … … r := … END\n[whole, rem] = DivMod(17, 5)' },
      { label: '[a, b] = f(x) (multi-call)', snippet: '[$0a, b] = fname(x)', description: 'Call a multi-output FUNCTION, binding each returned value to a variable (industry-standard-style destructuring).', usage: '[small, large] = Order(8, 3)' },
