...
export const FUNCTION_CATEGORIES: FunctionCategory[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'residue (partial-fraction / inverse Laplace)', snippet: 'CALL residue(num, den : r_r[1:$0], r_i[1:], p_r[1:], p_i[1:], k)', description: 'Partial-fraction (Heaviside) expansion of num/den: residues (r_r, r_i), the matching poles (p_r, p_i), and the scalar direct term k, so that num/den = sum r_i/(s - p_i) + k. The numeric inverse Laplace path — residues appear in the Solution window, and y(t) = sum r_i*exp(p_i*t). Distinct poles only.', usage: 'CALL residue(num[1:1], den[1:3] : r_r[1:2], r_i[1:2], p_r[1:2], p_i[1:2], k)' },
