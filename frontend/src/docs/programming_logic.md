...
## Functions
A `FUNCTION` is a local sequential program that returns one or more values. Inside functions, equations are solved **imperatively** using assignment `:=` (like Python/a matrix language).
- **Single-Output:** Returns a single value. Assign the function name to the return value inside the body.
- **Multi-Output:** Returns multiple values (industry-standard-style). Declare return variables in brackets in the header.
