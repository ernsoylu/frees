...
Unlike traditional programming languages (like Python, a matrix language scripts, or C++), where you must explicitly write equations in assignment form (e.g., `x = y + 2` to calculate `x`), frees is **declarative**. You write equations exactly as they appear in engineering textbooks:
