...
`tf(num, den)` builds a transfer function `num(s)/den(s)` from coefficient arrays in **descending powers** (industry-standard-style): `[1, 3]` is `s + 3` and `[1, 3, 2]` is `s^2 + 3*s + 2`. Use it on the left of an identity instead of writing the fraction out:
...
- **Transfer Function (TF)**: Represented as a pair of coefficient arrays in descending powers (industry-standard-style). E.g. `num = [0, 0, 1]` and `den = [1, 3, 2]` represents the system:
  $$G(s) = \frac{1}{s^2 + 3s + 2}$$
...
## Multi-Output Functions (industry-standard-style)

Every multi-output `CALL` function below also has a **destructuring** form — the same syntax a matrix language uses. Write the outputs in brackets on the left and call the function on the right; it is exactly equivalent to the `CALL name(inputs : outputs)` form, with output sizes still inferred:
...
Loop-shaping tuning of a P/PI/PID controller for a SISO plant `num/den`. The controller is designed so the open loop crosses over (gain = 1) at frequency `wc` with a 60° phase-margin target (the same default a matrix language uses). The type is a quoted `'P'`, `'PI'`, or `'PID'`; unused gains are returned as `0`. A pure `P` controller only sets the crossover — it cannot reshape phase.
```
