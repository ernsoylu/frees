...
frees uses a matrix language-like syntax for matrices and vectors. **You must declare the shape with a slice suffix** so the compiler can size the variable — the literal alone isn't enough.
