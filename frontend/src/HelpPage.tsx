...
function UnitsReference() {
  ...
  return (
    <Stack gap="md">
      ...
      <Text size="sm" c="dimmed">
        Use these anywhere in equations by their <code>#</code>-suffixed name (a reference solver convention).
...
h_pump = h_total - h_inlet`,
  },
  {
    ...
    note: "Results: total heat load = 250,000 Btu/hr, supply enthalpy = 20.25 Btu/lb, supply wet-bulb temperature = 55.0°F. (licensure-exam Problem 9)",
...
y_test = siyavula_data(0)`,
  ...
  // Sourced from the a reference solver Clone research corpus (a reference solver manual / the standard methodology text /
  // the standard literature) and verified in the backend OdeProblemLibraryTest against closed-form
...
const CATEGORIES = [
  ...
  },
  {
    ...
    items: [
      { id: 'matrices-decl', label: 'Declaring Matrices & Vectors', keywords: ['matrix', 'vector', 'declaring', 'literal', 'semicolon', 'brackets', 'matrix-language'] },
      { id: 'matrices-ops', label: 'Matrix Operators (+, -, *, \\, \')', keywords: ['operators', 'transpose', 'backslash', 'multiplication', 'solve', 'matrix-language'] },
      ...
      case 'matrices-decl':
        return (
          <Stack gap="md">
            ...
            <Title order={3}>a matrix language-Style Matrix Literals</Title>
            ...
            <Text mt="xs">
              You can use <strong>bare names</strong> (a matrix language style) — <code>A = [3 4 5; 1 2 6; 7 8 1]</code> creates the matrix and <code>SolveLinear(A, b)</code>, <code>inv(A)</code>, <code>A * b</code> all accept it directly — or the explicit indexed form <code>A[1:3, 1:3] = …</code> when you want to address elements.
            </Text>
            ...
            <List size="sm" withPadding>
              ...
              <List.Item><code>inv(A)</code> = Inverse, <code>det(A)</code> = Determinant (a matrix language aliases)</List.Item>
            </List>
            <Paper withBorder p="md" bg="light-dark(var(--mantine-color-gray-0), var(--mantine-color-dark-8))">
...
g = linspace(0, 10, 5)`}</Code>
            ...
            <Text size="xs" c="dimmed" mt="xs">
              Note: frees is a declarative solver, so it uses bracket indexing with ranges (<code>A[i, j]</code>, <code>A[1:n]</code>) rather than a matrix language's parenthesis indexing and <code>A(:, 2)</code> / <code>end</code> slicing.
            </Text>
          </Stack>
        );
      case 'matrices-ops':
            ...
            <Table striped highlightOnHover withTableBorder mb="md">
              ...
              <Table.Tbody>
                ...
                <Table.Tr>
                  ...
                  <Table.Td>industry-standard-style <strong>element-wise</strong> multiply, divide, left-divide, and power. A scalar on either side broadcasts to every element.</Table.Td>
...
END`}</Code>
            ...
            <Title order={3} mt="sm">Multi-output functions (industry-standard-style)</Title>
            <Text style={{ lineHeight: 1.6 }}>
