...
- **Story 8.7: Curve Tables.** A Curve Table holds a named function: argument name(s), result name, axis units/log flags, and one (x, y) column pair per curve with the curve's parameter value in the header (e.g. `T = 100`, `T = 200`, …). Filled by "Send to Curve Table" from the Digitizer or edited manually; rows are kept sorted by X.
